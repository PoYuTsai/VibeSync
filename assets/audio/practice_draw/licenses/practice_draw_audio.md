...
- **bundle 狀態**：抽取需 ffmpeg（WSL 待 `apt install ffmpeg`）。code 已 code-complete；mp3 落地後本表更新。
...
- 目標：優先小檔案，整組翻牌音效資產 **< 500KB**（E2 退役 riser/settle≈−192KB→現 ≈ 296KB；待加 bed mp3 ~9s ≤128kbps ≈ +144KB → ≈ 440KB）。
...
| `playRevealBed()` 揭曉配樂 bed（一次性，~9s，與 `_reveal` 同長） | `practice_draw_reveal_bed.mp3`（待 ffmpeg 抽取） | `音檔.mp4` 音軌 0–~9s 全段（Gemini 生成參考影片）+ ffmpeg normalize | 夥伴用 Google Gemini 生成（for VibeSync）+ ffmpeg 後製 | 專案原創（AI 生成，非第三方 sample） | 2026-06-27 | 否 |
...
- [ ] **E2 抽取 bed mp3**：ffmpeg 抽 `音檔.mp4` 0–~9s、normalize、編 mp3 ≤128kbps、放進
      `assets/audio/practice_draw/practice_draw_reveal_bed.mp3`，加「bed asset 存在且非空」測試。
