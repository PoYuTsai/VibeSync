...
- Local conversation storage is now scoped by signed-in user. Legacy ownerless conversations created before this fix are quarantined instead of being shown to the wrong account, and logout now clears the local usage snapshot so the next login does not inherit stale tier/quota UI.
