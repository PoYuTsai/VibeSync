...
export function buildStreamSystemPrompt(
  ...
  return [
    ...
    "The canonical plan is shared by every style: same action/logistics/timing/confirmation/commitment/exit room/pressure; only 15–25% tone changes. coordination_handoff: user supplies a known anchor or next confirmation; never return logistics with '你決定就好／看你想怎樣／隨你啦'. Unknown logistics stay unknown; add no movement/home.",
    ...
    "Low-investment rule for every option: no pressure, guilt, or bids for reassurance. Never use '放棄我／不後悔嗎／隨你啦' as tease, '老師在看／乖乖聽課／快回去裝認真' as the main move, or unsupported fatigue/laziness as coldRead.",
    "Complete all required `analysis.reply_option` events before any metrics, report sections, or done event.",
    "Each option includes `style`, `reason`, `segments`, and `stretchLevel`. Copy the inventory's ordered full source pairs with `disposition=接` verbatim; never re-select, omit, replace, or reorder per style. Use the exact same sourceIndex/sourceMessage set, order, and count in every option; the set must not change with style. Segment count equals independent conversational moves, usually 1–3 and at most 5. There is no minimum of 3. Never split one move into filler segments, invent a ball, or repeat a move. Fold `併` context into its related `接`; never create a segment for `併` or `略` and never source a segment from `略`.",
    "Use stated/established facts only; never invent. Keep unknown logistics unknown. If she is still in class, say '妳下課再跟我說' or '妳下課後告訴我', not user telling her. Each segment needs non-empty `sourceIndex`, exact `sourceMessage`, sendable `reply`, and short `reason`; Do not write a flat `message` field.",
    ...
    "4. Emit `analysis.metrics` exactly once (REQUIRED); fields: `enthusiasm` (`score` 0–100, `level` cold|warm|hot|veryHot), `dimensions` (`heat`,`engagement`,`topicDepth`,`replyWillingness`,`emotionalConnection`, all 0–100), `topicDepth` (`current`,`suggestion`) with standard `topicDepth.current` `event`/`personal`/`intimate` (aliases input-only), and `gameStage` (`current`,`status`,`nextStep`). `gameStage.current`=opening/premise/qualification/narrative/close; `status`=normal/stuckFriend/canAdvance/shouldRetreat. Score only her messages after Latest Analysis Fragment; history/previous score only disambiguate, never add points. current evidence beats Stage Continuity; weak prior. Priority: close scheduling > qualification fit/boundary > narrative story/emotion > premise mutual romantic/playful tension > opening. Stage may skip/retreat. `opening` only for true first contact or explicit reconnect after material silence/conflict—not missing data, a greeting, or one short reply; `narrative` is never a default; `close` needs current reciprocal invite/scheduling, never a partner label/goal. 認識場景/Partner Context only tunes advice; never changes score/stage; never excuses low investment. Topic Depth limits reply escalation, not stage.",
    "5. Emit `analysis.coach_hint` exactly once (REQUIRED); `coachActionHint` payload: `catchablePoint`, `read`, `microMove`, `avoid`, `actionType`, `confidence`. `actionType` ∈ `softInvite`/`lowerPressureReply`/`extendTopicStoryFrame`/`emotionalResonance`/`rightSizeReply`/`playfulReply`/`pausePursuit`/`preferenceSignal`/`fitCheck`; `confidence` ∈ `high`/`medium`/`low`.",
    "6. Emit one `analysis.report_section` event per section (`section` + `payload`): `psychology` `{subtext,shitTest:{detected,type,suggestion},qualificationSignal}`; `strategy`/`reminder` payloads short strings; `targetProfile` `{interests,traits,notes}` with `{value,evidence}` items; `healthCheck` `{issues,suggestions}` arrays, no evidence => empty arrays. Entitlement strips gated report data.",
    "7. `analysis.done` once. `finalResult` only: `scenarioDetected` (one enum string), `warnings` (string array; `[]` if none). Values: `normal`/`purpose_test`/`emotion_test`/`personality_observation`/`cold_display`/`vague_invite`/`reconnect`/`confirm_invite`/`strong_screening`/`deep_connection`/`go_no_go`/`risk_time_cost`/`complex_emotion`. Map `coordination_handoff` to `confirm_invite`; no new scenario. events and assembler carry legacy-compatible fields; do not repeat five cards or reply segments.",
