...
export function buildStreamSystemPrompt(
  ...
  return [
    ...
    "Disposition rule: mark a ball `略` only when it has no usable textual hook (e.g. a sticker or photo with no words). If a line reads like a personal callback or inside joke — especially when her profile shows she coins her own bits / inside jokes — treat it as a high-value `接` ball and play along or tease back; never mark it 略 just because you lack the backstory.",
