...
const DIVERGENCE_METHOD_LIST = DIVERGENCE_METHODS.map((method) =>
  ...
  .join("/");
const DIVERGENCE_PLAN_FIELD_LIST = DIVERGENCE_PLAN_REQUIRED_EVENT_KEYS.map((
  field,
) => `\`${field}\``).join(", ");
const DIVERGENCE_PLAN_OPTIONAL_LIST = DIVERGENCE_PLAN_OPTIONAL_EVENT_KEYS.map((
  field,
) => `\`${field}\``).join(", ");
...
const DIVERGENCE_PLAN_STEP = [
  ...
  `Meaning: \`schemaVersion\` 1; \`threadFrame\` (one line: the single thread the whole reply set follows); \`anchorSourceIndex\` (the \`接\` ball with the most interaction value: it alone may carry the forward hook, the new picture, or the single question); \`supportSourceIndices\` (other \`接\` balls: catch them with a statement, feeling, or short stance, never each with its own question); \`mergeContextSourceIndices\` (the \`併\` balls); \`semanticDistanceCap\` 0-${MAX_SEMANTIC_DISTANCE_CAP}, \`newTopicBudget\` 0-${MAX_NEW_TOPIC_BUDGET}, \`questionBudget\` 0-${MAX_QUESTION_BUDGET} (red/falling or low investment: all 0; green/rising: cap ${MAX_SEMANTIC_DISTANCE_CAP}, budgets ${MAX_QUESTION_BUDGET}; scheduling: cap 1, new topic 0); \`branchPool\` (${MIN_DIVERGENCE_BRANCHES}-${MAX_DIVERGENCE_BRANCHES} branches: \`id\` (opaque \`br_1\`, \`br_2\`, … only; never words), \`sourceIndex\` (exactly one, never a numbered variant like \`sourceIndex1\`), \`method\` one of ${DIVERGENCE_METHOD_LIST} (how you branched; only these six, so a humor branch built by exaggeration is still \`association\` or \`affect_evaluation\` here), \`idea\`, \`associationPath\` (the visible chain from her words to the idea; a branch without a grounded path is not allowed), \`semanticDistance\` 0-${MAX_SEMANTIC_DISTANCE}: 0=her exact ball, 1=same scene or direct detail, 2=grounded detour or callback, ${MAX_SEMANTIC_DISTANCE}=far leap, which is always above the cap, so list it only to show a path you rejected); \`styleBranchIds\` (optional) mapping each reply style to the branch id it will use.`,
...
// §14.1）。欄位名、每風格手法值域、強度上限全從 divergence_contract 生成。
const REPLY_OPTION_BRANCH_FIELD_LIST = REPLY_OPTION_BRANCH_FIELDS.map((
  field,
) => `\`${field}\``).join(", ");
const STYLE_MOVE_LIST = (Object.keys(STYLE_RHETORICAL_MOVES) as Array<
  keyof typeof STYLE_RHETORICAL_MOVES
>).map((style) =>
  `${style}: ${
    STYLE_RHETORICAL_MOVES[style].map((move) => `\`${move}\``).join("/")
  }`
).join("; ");
const DIVERGENCE_REPLY_OPTION_RULE = [
  `Branch attribution: when you emitted \`${DIVERGENCE_PLAN_EVENT_TYPE}\`, every \`analysis.reply_option\` also carries exactly ${REPLY_OPTION_BRANCH_FIELD_LIST}: \`selectedBranchIds\` is a non-empty array of \`branchPool\` ids this style actually follows (its own \`styleBranchIds\` entry when you gave one; the anchor's branch otherwise); \`rhetoricalMove\` is how this style phrases the branch, either one of the six branch methods (${DIVERGENCE_METHOD_LIST}) or one of that style's own moves (${STYLE_MOVE_LIST}); \`styleIntensity\` is 0-${MAX_STYLE_INTENSITY} (lower it when the style does not suit the moment; never change the action instead). Give different styles different branches or different moves: five options that share one opening, or differ only by synonyms and punctuation, are a failure. The branch changes only the angle inside the anchor segment; every option still covers the same \`接\` balls, keeps the shared action, and stays inside \`questionBudget\` and \`semanticDistanceCap\`. If attribution is missing or invalid, the server uses your \`styleBranchIds\` entry for that style, else the anchor's branch. If you emitted no plan, omit these keys and write the options as before.`,
  'Example attributed reply_option line: {"type":"analysis.reply_option","style":"humor","selectedBranchIds":["br_2"],"rhetoricalMove":"exaggeration","styleIntensity":2,"reason":"把火鍋當熱量進出帳來玩","stretchLevel":"stretch","segments":[{"sourceIndex":1,"sourceMessage":"剛健身完","reply":"肌肉剛下班，妳本人還沒","reason":"接健身球"},{"sourceIndex":3,"sourceMessage":"等等要去吃火鍋","reply":"等等火鍋再把今天消耗的熱量全部請回公司","reason":"接火鍋球"}]}',
  // Phase 3b 黑箱：加「開頭規則」對同開頭無效（4 vs 3 案），只多 250 tokens，
  // 已撤；同開頭改由 telemetry sameOpeningCount 量，規格字面 gate 是 ≥4 張才違規。
  "questionBudget counts every question in the whole option, across all its segments, not per segment.",
];
