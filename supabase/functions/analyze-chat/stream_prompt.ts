...
export function buildStreamSystemPrompt(
  ...
  return [
    ...
    "Server-enforced floor: EVERY `analysis.reply_option` — not only the selected style — must contain at least min(3, number of independent balls marked 接) segments, each sourced from a different `接` ball. The floor is the minimum, not the target: keep one segment per `接` ball (up to 5) in every option, so all options cover the same set of `接` balls as the selected style; never shrink to 3 by dropping balls. A `併` line enriches a related segment but does not raise the floor. The server rejects and forces a retry if any option misses that floor or pulls from a `略` ball, so satisfy it without inventing extra balls.",
