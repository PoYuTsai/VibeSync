...
Deno.test("prompt 示範句／schema placeholder 照抄 → 永遠丟（不需呼叫端傳入）", () => {
  for (const example of FORMULA_PROMPT_EXAMPLE_LINES) {
    assertEquals(
      normalizeFormulaReplies([{ openingLine: example, whyItWorks: "理由" }]),
      [],
      `openingLine 示範句應永遠排除：${example}`,
    );
  }
  for (const note of FORMULA_PROMPT_PLACEHOLDER_NOTES) {
    assertEquals(
      normalizeFormulaReplies([
        { openingLine: "正常的公式開場句子", whyItWorks: note },
      ]),
      [],
      `whyItWorks placeholder 應整則丟：${note}`,
    );
  }
  // 真實教練註解含「教練」字樣不誤殺（只擋 dedupe key 完全相同）。
  assertEquals(
    normalizeFormulaReplies([{
      openingLine: "正常的公式開場句子",
      whyItWorks: "教練註解：這句接住她的咖啡線索，她補店名就能回。",
    }]).length,
    1,
  );
});
