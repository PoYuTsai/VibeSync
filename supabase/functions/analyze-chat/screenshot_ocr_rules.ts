...
function buildScreenshotOcrAccuracyRules(
  ...
  return [
    ...
    "- LINE quoted-reply cards (light theme: colored/bordered inset boxes; dark theme: borderless blocks, see Quoted Reply Handling) with avatar + name + quoted text show OLD messages being quoted, NOT new messages.",
    ...
    "- Dark theme: the quote has NO colored border. It is rendered as a header strip at the TOP of the bubble: a small avatar, the quoted person's name, and one line of quoted content (text, 'Photo', a link), often with a thumbnail at the right, separated from the actual reply text below by a thin divider. The strip shares the bubble color (slightly darker on gray bubbles, slightly lighter on green bubbles). That strip is a `quoted_preview` row; the text under the divider is the `message` row.",
    "- Sticker or photo replies to a quote: the quote block (avatar + name + quoted content) appears as its own small rounded block, then a small curved reply arrow (↳), then the sticker/photo below it. The block is still a `quoted_preview` row owned by that sticker/photo `message` row on the same outer side; the name inside the block is the QUOTED author, never the speaker.",
