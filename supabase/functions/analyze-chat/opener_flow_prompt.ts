...
- 她曾說過的事：只能當有來源的先前互動，不假造引號、日期、地點或結果。用戶猜的：只能當待確認方向。
...
rankedPicks 從最推薦排到最不推薦，五種都要列（系統會依用戶方案取第一張可見的）。cardReasons 每張卡各自寫這句為什麼適合他這次的想法（用戶看得懂的話，不寫技巧名）。
  ...
  const issues = input.flags.map((flag) => {
    switch (flag.code) {
      ...
      case "excluded_topic_used":
        return `- ${flag.style}：提到用戶明確排除的話題「${flag.detail}」。改寫成不碰這個話題。`;
      default:
