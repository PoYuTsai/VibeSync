...
- **她主打的照片很露時，兩條路**（Eric 2026-08-19 拍板）：
  ...
  - 這招的命門是**可否認性**：一旦補上 😏、加註解、或多寫一句解釋，它就從雙關掉成猥褻，比直白更難看。**不解釋、不加暗示型表情符號、只寫一則就收**。做不到乾淨就走上面那條路，別硬玩。
...
- **可以分兩則**（2026-08-19 Eric：對標練習室，高手不把一坨字擠一行）：用**真的換行**分開，最多兩則、不可用「｜」「/」代替。分則時每則 6-15 字、整串仍在 25 字內；第一則丟觀察或判斷，第二則接小挑戰、小玩笑或一個好接的開口。不確定就一則，別硬湊。
...
- 斷句用空格就好，不用硬加標點；語助詞挑一個（欸／喔／齁／啦），不要連用。
