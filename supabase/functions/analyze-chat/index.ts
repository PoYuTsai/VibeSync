...
每次都先完成以下判斷，再生成開場白：
1. **先避開 avoidTopics**：自介或用戶描述中明確寫「不要問、討厭、不喜歡、不喝、沒誠意、不要查戶口」等，必須列出並避開。這些是最高優先級，不能踩。
2. **判斷框架 frameRead**：讀懂她的語氣與界線，但不要跪著回。界線要被內化，不一定要被唸出來；開場白不是向她交作業。
3. **可接線索 positiveHooks**：找出對方主動給出的可回線索，例如「熱愛學習嘗試新事物」「F1」「樂華夜市」「狗狗」「登山」「剛看完比賽」。這些比外貌稱讚更重要。
4. **好奇心鉤子 curiosityHook**：設計一個小反差、小畫面、小幽默、小選擇題或輕微挑戰，讓她有動機回。鉤子要自然，不能變油、不能硬演。
5. **推薦策略 openingStrategy**：一句話說明「避開什麼、接哪個線索、用哪個好奇心鉤子、保留什麼個性」。推薦的開場白必須能看出這個策略。
...
## 自介資訊量分流
- 自介很長、界線很多：不要逐條回覆，不要像客服。抓「一個界線 + 一個正向線索」即可；開場白要短、有分寸、有一點個性。
