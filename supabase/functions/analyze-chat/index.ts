...
function buildImageAnalysisPrompt(options: {
  ...
  return joinPromptSections(
    ...
    compiledConversationText
      ? `## Existing Thread Context\n${compiledConversationText}`
      : "",
    "### Multi-Message Reply Reminder\n- 截圖中如果對方連發多條訊息，finalRecommendation.content 必須逐條檢查並分句標註回覆（① 回「關鍵詞」→ 回覆）。不要只回最後一條，中間有好的接話點也要回。",
  );
}
...
4. 用戶定位：套用 About Me / Partner Style 的語氣與練習目標，但不要替用戶假裝成另一個人
5. 下一步選擇：收、接、延伸、篩選、邀約、暫停，選一個最小動作
6. 生成回覆：像真人訊息，短、自然、可直接複製；不要輸出內部術語
...
2. 對方是否明顯無興趣或低投入
3. 是否有明確邀約或推進窗口
4. 是否只是輕鬆接球、人格觀察、話題延伸
5. 最後才選擇技巧型回覆，不要為了技巧犧牲自然
...
- 策略: 不默認鼓勵赴約。先判斷這局是否清楚、健康、值得投入，再建議低成本釐清或婉拒
...
- 若用戶帶著性羞愧: 先提醒「有慾望很正常」，不需要把自己想靠近、想親密視為錯；真正需要練的是清楚、尊重、有責任感地表達
...
- No-Go: 對方低投入、只索取情緒價值、第三方關係不清、時間成本高但回報低、讓用戶明顯失去穩定感
...
## 輸出格式 (JSON)
{
  ...
  "scenarioDetected": "normal | purpose_test | emotion_test | personality_observation | cold_display | vague_invite | reconnect | confirm_invite | strong_screening | deep_connection | go_no_go",
