...
共同病灶與硬規則：
1. **五句的句式必須彼此不同**。特別禁止全部落在「A 但 B」「A，其實 B」這種前提＋轉折的骨架裡；最多一句可以用。
...
- **預設一則；只有真的有兩個獨立動作才分**（2026-08-19 Eric：一句講得完就一句，看情境，不要寫死）：分兩則的唯一理由是「第一則丟出去、第二則是另一個動作」（觀察→挑戰、判斷→玩笑）。同一個意思被逗號切成兩段硬換行＝更僵硬，不如一句。分則用**真的換行**、最多兩則、不可用「｜」「/」代替，每則 6-15 字。五種風格裡通常只有一兩種需要分。
