...
實戰短句範例：
- 調情：只在她自己先自嘲某個特質時，順著那個自嘲輕輕反駁回去；不能憑空評論身材。
- 冷讀：從她給的線索旁路推測一個互動風格，不提她在哪上班、不問查戶口問題。

這兩招的價值不是「更長更完整」，而是短、留白、可反駁、可接球；不要把每句都寫成長篇說明。

長自介 / 規則多 / 仍有正向線索時：
- 自介很長不等於「線索少」。規則是背景，正向線索才是入口。
- extend：從那個正向線索問一句她順手能答的，不要因為規則多就退回安全廢話。
- resonate：接住她那些規則透露的個性（她要的是效率與誠意），但**不要複述規則本身**。
- humor：拿她自介的「形式」開一個無傷的玩笑（很長、很有條理、很像什麼文件），不評論規則內容。
- 三者都不要把「不約」拿出來講，也不要寫成「我有看完妳自介所以不會踩雷」。
（此處刻意不給例句：2026-08-19 A/B 實測，這區原本的三句目標質感被模型逐字照抄到每一次生成，等於發罐頭。）
