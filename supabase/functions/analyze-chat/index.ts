...
function buildImageAnalysisPrompt(options: {
  ...
  return joinPromptSections(
    ...
    compiledConversationText
      ? `## Existing Thread Context\n${compiledConversationText}`
      : "",
    "### Multi-Message Reply Reminder\n- 截圖中如果對方連發多條訊息，先判斷哪些球值得接。中文問句不一定都是必答題；先分辨真問題、情緒球、框架測試或玩笑反問，再決定答、半答、重框、略過或反丟。finalRecommendation.content 是最推薦的訊息組文字，可用換行表示 2-3 則真人訊息，但不要放 ①② 標註或「回某句」報告格式；對方連發 2 顆以上值得接的球時，必須填 finalRecommendation.replySegments 一球一段（最多 3 段，每段必填 sourceIndex 與 sourceMessage），讓 App 顯示引用原句與分段複製。replyOptions 則要提供五種風格各自的「接法 + 訊息組」。finalRecommendation.reason 再簡短說明接了哪些球、略過哪些低價值資訊。",
  );
}
