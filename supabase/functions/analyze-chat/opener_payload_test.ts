...
Deno.test("raw formulaOpeners 絕不隨 spread 穿透（normalizer＋filter 兩層剝除）", () => {
  const parsed = {
    openers: { extend: "延展句" },
    recommendation: { pick: "extend", reason: "理由" },
    formulaOpeners: [{ openingLine: "raw 公式", whyItWorks: "raw 理由" }],
  };

  const normalized = normalizeOpenerPayload(parsed);
  assertEquals(normalized === null, false);
  assertEquals(
    "formulaOpeners" in (normalized as Record<string, unknown>),
    false,
    "normalizeOpenerPayload 必須剝除 raw formulaOpeners",
  );
  ...
  assertEquals(filtered === null, false);
  assertEquals(
    "formulaOpeners" in (filtered as Record<string, unknown>),
    false,
    "filterOpenerPayloadForAllowedFeatures 必須剝除 raw formulaOpeners",
  );

  // 50-item 洪水陣列同樣不得穿透。
  const flooded = {
    openers: { extend: "延展句" },
    formulaOpeners: Array.from({ length: 50 }, (_, i) => ({
      openingLine: `洪水${i}`,
      whyItWorks: "x",
    })),
  };
  const floodedFiltered = filterOpenerPayloadForAllowedFeatures(
    flooded,
    ALL_FEATURES,
  );
  assertEquals(
    "formulaOpeners" in (floodedFiltered as Record<string, unknown>),
    false,
  );
});
