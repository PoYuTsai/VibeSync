...
Deno.test("validateSelectedSegments does not 誤殺 segments whose sourceIndex is absent from inventory", () => {
  const inv = inventoryOf([[3, "接"], [4, "接"], [5, "接"], [6, "接"]]);
  // index 9 not in inventory → treated as allowed, not 略.
  assertEquals(validateSelectedSegments(inv, [seg(4), seg(5), seg(9)]), { ok: true });
});
