...
- direction 是客戶看到的卡片標題：只說這張卡要聊什麼，不要寫「切角」「選項」「方法」或生成過程（一句話，≤35 字）。
...
- whyItWorks 與 nextMove 都用一般人看得懂的話：whyItWorks 說明她為什麼好接；nextMove 說她回了之後具體怎麼延續。不得出現內部方法名、欄位名、狀態代碼或生成過程；不要用「這題使用了某技巧」的口吻。nextMove 要可執行、具體、不情勒。
...
- 不要重新發想，不要新增原文沒有的內容。
- 若原文已有可用的 openingLine，openingLine 必須逐字保留，順序也不得改；只改寫客戶看到的解釋，不重寫可直接傳的句子。
...
- direction / whyItWorks / nextMove / recommendation.reason 都是客戶會看到的話；不得留下內部方法名、欄位名或「切角」式生成說明。
...
export function buildNewTopicUserPrompt(input: {
  ...
  if (input.requestId) {
    sections.push("");
    sections.push(
      `## 本輪內容素材（只供發想，不得照抄）：${pickNewTopicAngle(input.requestId)}`,
    );
    sections.push(
