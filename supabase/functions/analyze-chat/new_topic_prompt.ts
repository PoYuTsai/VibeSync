...
- openingLine：可以**直接傳出去**的第一則訊息（繁體中文、台灣自然語感）。**10-25 字，超過 30 就是在寫作文，上限 35**——長度本身就是位階訊號，要像順手丟的，不像熬夜寫的（2026-08-19 Eric 拍板）。陳述句收尾優先、不一定要問號（下一個她想澄清的判斷比問句更容易被接，問號容易變面試）；五題至多兩題以問號收尾。**可以用真換行分成最多兩則**（對標練習室：高手不把一坨字擠一行；不可用「｜」「/」代替），分則時每則 6-15 字、整串仍在 25 字內，第一則丟觀察或判斷、第二則接小挑戰或好接的開口。不是教練說明、不是模板、不含「你可以說……」這類框架語。
