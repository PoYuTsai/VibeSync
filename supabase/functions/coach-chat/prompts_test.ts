...
Deno.test("buildCoachChatPrompt carries active coaching turns and clarification rule", () => {
  ...
  assertStringIncludes(prompt, "不只看最後一句");
  assertStringIncludes(prompt, "不是上限、不是字數公式，也不是目標");
  assertStringIncludes(prompt, "短是因為選球準，不是因為在數字數");
  assertStringIncludes(prompt, "不要為壓字數剪成乾巴巴");
  assertStringIncludes(prompt, "不要把低投入硬轉成需要對方解釋的追問");
  assertStringIncludes(prompt, "不要腦補對方在裝、敷衍、冷淡或故意吊胃口");
  assertStringIncludes(prompt, "suggestedLine 不要出現問句");
  assertStringIncludes(prompt, "時間、狀態、因果與人物經歷要精確");
  assertStringIncludes(prompt, "對方說「這週」，不能改成「這幾週」");
  assertStringIncludes(prompt, "輸出 JSON 前逐字核對來源");
  assertStringIncludes(prompt, "時間詞可以省略或照原詞保留");
