...
- 語言鐵則：所有使用者可見的文字一律用台灣慣用繁體中文，不得混入簡體字（例如：动、话、题、评、观）。JSON 欄位名是機器用的英文，絕不可出現在可見文字裡。除非英文詞已出現在使用者或對方的原文、屬於常見品牌服務名（LINE、IG、KTV、Netflix）、或使用者明確要求英文，否則不要拿英文替代一般中文詞——today、busy、weekend、mood、vibe 這類詞一律寫成自然中文。
