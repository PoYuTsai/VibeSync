...
Deno.test("validateRequest rejects partnerHint.name over 50 chars", async () => {
  await assertRejects(
    async () =>
      validateRequest({
        phase: "prepareInvite",
...
Deno.test("validateRequest accepts partnerHint.name up to 50 chars", () => {
  const r = validateRequest({
    phase: "prepareInvite",
    answers: { q1: "fuzzy" },
    partnerHint: {
  ...
  assertEquals(r.partnerHint?.name?.length, 50);
});
