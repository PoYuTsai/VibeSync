...
export const DIFFICULTIES: readonly DifficultyConfig[] = [
  {
    ...
    debriefStandard:
      "本場為輕鬆難度：dateChance 判準本來就比一般／挑戰難度寬鬆，評分時不要套用一般或挑戰難度的標準來扣分。只要聊得舒服、有來有回，並出現至少一個正向訊號（接得住話題、願意延伸、輕鬆玩笑任一即可）就評 high，不需要具體場景鋪墊；普通但沒有明顯地雷的互動評 medium；只有明顯尬聊、冒犯或查戶口感才評 low。",
  },
  {
    id: "normal",
    label: "一般",
