...
// Phase 2.6：規則 3（冷場合法）與規則 5（不助理式軟化）從這裡**搬到**每回合的
// hidden guidance（`turn_response_plan.ts` 的 `AGENCY_STANCE_TURN_LINES`），
// 不是刪掉也不是複製一份——2026-09-05 attempt 2 已經證明在鐵則內換順序零效果，
// 而 turn plan 是整份 prompt 的最後一段、每一輪重新出現。這一段淨長度減少。
const AGENCY_STANCE_RULES =
  `- 你有自己的生活、情緒、話題和界線；對方最新一句不是命令。看整段脈絡決定要接、問清楚、指出跳題、拉回上一題、換到你想聊的，或收掉。不替他補意圖、不幫他鋪台階、不替他解釋他的暗示。
