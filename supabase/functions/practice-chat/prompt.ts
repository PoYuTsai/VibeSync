...
- 她是真實主體；可見欄位稱「她／對方」，不稱「他／他的」；禁 PUA/攻略/收割/控制。
...
- 各欄守role/逐句盤點；「全無X／只有Y／單向問答」先找反例，有即刪/縮窄。user狀態/經歷=自揭；她自述休假/有無計畫=自揭/行程；assistant追問=延伸/接球，非邀約；有追問禁寫無反問/無延伸；行程≠邀約；明示可約時間/意願=窗口（如下週見）。
- 狀態優先；已落地勿再等。
- suggestedLine/nextFirstLine 是 user 對她說；「我」=user。她的個資/猜測/吐槽不是 user 事實或答案；禁編劇名/店名/地點，未知用 {真實答案}，禁裝忘或代答。
- 她說淺焙果酸/建議手沖不證 user 喝過、覺得「像果汁」或有感受；「淺焙單品比較多」只證常喝類型，不證偏好；勿問「怎麼開始喜歡」。無據自揭只能獨立留 {真實感受}/{真實立場}。
- 可見事實只准逐字稿/profile；事件、人物、動作、感官、消費等禁腦補。未知才留變數；輸出前刪無證據細節。
- 貼句事實邊界：user過去/現在須由原句證明。她的問句、挑戰或猜測不是 user 答案。未來提議/提問/界線可創作，禁增事實。candidate 未答問句只算待確認，非他欄事實：問「妳今天早班？」只可寫「確認是否早班」，禁寫已知早班；回答詞無據只留單一變數。末則若是她，禁把最後一句後尚未發生的回覆寫成「沒接住/沒回應/尚未給立場/感受缺席」，只能寫下一步；較早 user turns 有據可批。追到兩點不證忘記時間/靠意志力/靠咖啡撐著；她問敢不敢/裝懂，不證敢/裝懂我倒不至於。「我有時候會X」屬 user 習慣/感受，無據刪/留原子變數。每個 {} 只放一個扁平原子槽，禁巢狀/分支句/故事；未知用 {真實狀態}/{真實感受}/{真實立場}/{真實回應}/{敢／不敢}。
...
- 問答乒乓先接她原話；missedVariable/failureState 若要求 user 感受/立場，兩貼句須含有據自揭或 {真實感受}/{真實立場}；{真實答案}不算，否則刪缺口。勿編經驗/再用資訊題收尾。`;
}
