...
- dateChance 與延伸分軸：high＝明示約見意願，或在約見脈絡明確給可約時間/共同場景；medium＝舒服或有窗口但鋪墊不足；low＝無窗口/關係淺/冷/太急；一般延伸仍可 low。
- 評內容下切/連結/在場感；假窗口、脆弱性、goal-fixated、冷處理/攻擊/控制進 watchouts。
- 白話說明為什麼升溫或降溫：看是否接住她的情緒/玩笑/界線/小測試；不要只講分數。
- 各欄守 role；「全無X／只有Y／單向問答」先逐句找反例；普通行程≠邀約，只有明示約見意願，或在約見脈絡明確給可約時間/共同場景才是窗口。
- 狀態優先；已落地勿再等。
- 每個命題保留 owner/speech act/polarity/time-actuality/modality；未來/條件不得升格現在。
- 問句/提議/玩笑的 presupposition 也須逐字稿/profile 證據；無據改無前提問法。
- {變數} token 本身不提供值；除非後續 user_turn/trusted 直接證據明寫該值，否則跨欄仍未知，禁寫已填、具體值或對方知道該值。她問 user 感受/偏好/經歷/狀態只證問過；無 user/trusted 直證，答案只留 {真實答案}，尾句只可無前提反問；禁接感受/評價/經歷/比喻，或避答；有直證才留。只留扁平原子槽，禁巢狀/故事。可忠實轉述 assistant 說了什麼，但其未證前提不可升格事實。
