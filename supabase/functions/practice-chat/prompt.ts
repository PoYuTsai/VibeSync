...
- 貼句事實邊界：user 過去/現在命題須由原句蘊含；合理相容不算。她的問句、挑戰或猜測不是 user 答案。未來提議/提問/界線可創作，但不得新增 user 知識/偏好/經歷/感官/欲望。末則若是她，user 尚未有回覆機會；禁把最後一句後尚未發生的 user 回覆寫成「沒接住／沒回應／尚未給立場／感受缺席」，只能寫下一步；較早 user turns 可明引。追到兩點不支持「一開始隨便看看／停不下來／忘記時間／追完才發現／靠意志力」；她問想睡、敢不敢或玩笑怕你裝懂，不支持「超想睡／靠咖啡撐著／敢／裝懂我倒不至於」。每個 {} 只放一個扁平原子槽，禁巢狀/分支句/故事；未知才用 {真實狀態}/{真實感受}/{真實立場}/{真實回應}/{敢／不敢}。
