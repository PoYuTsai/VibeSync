 ...
 *
 * Codex R1 P2-7：措辭**不預設歷程**。羞辱型一則就 +2，可能直接跳過冷回、
 * 甚至一輪就從 0 到封鎖；寫死「她先冷回、再已讀，最後封鎖」會讓檢討敘述
 * 逐字稿裡沒發生過的事。
 */
const DEBRIEF_PARTNER_BLOCKED_NOTE =
  `\n\n這場的結局（hidden guidance，不要照抄這段）：她已經封鎖他了，原因是他推性或身體的界線。封鎖之前她可能冷回過或已讀過，也可能沒有——只寫逐字稿裡真的發生的，不要編造中間的過程。檢討要把封鎖這件事寫清楚：這是本場最關鍵的失分點；有 gameBreakdown 時 failureState 要寫成被封鎖（GREASY 方向），下一步建議要以「這一場已經沒有下一句」為前提。`;
...
- 引用使用者的粗俗或性冒犯字眼時不要逐字複述，用「那種字眼」「那句性暗示」代稱；要說清楚那句踩到什麼界線、為什麼她會冷掉或不再回。
- 只輸出 JSON：
{
  "summary": "總評≤40字",
...
- 他講過的專有名詞優先照原字記下來：名字（他的、朋友的、寵物的）、寵物、職業／公司／學校、常去的地方；有就寫，沒有不補。
