...
`;
// 排序是刻意的（踩坑「prompt 規則堆太多後面幾條會被模型直接忽略」）：夥伴五條
// 規則排在鐵則**最前面**，長度最長、最吸注意力的台語對照段排在它們後面。
// 2026-09-05 attempt 1 把這三條排在台語段之後，助理式軟化 30%／鋪台階 25%
// 都沒壓下來；attempt 2 只換順序、一個字都不加（長度不變），單獨測「後面幾條
// 被忽略」這個假設。
...
${shape}${AGENCY_STANCE_RULES}${AGENCY_IRON_RULES_TAIL}${mood}${AGENCY_SURFACE_RULES}
