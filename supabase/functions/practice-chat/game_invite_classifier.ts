...
// 「跟我／跟妳＋一樣」是比較句（跟我一樣爛劇看到一半就關掉），不是兩人一起
// 做一件事。沒排除的話會被判成使用者主動軟邀約 → FSM 跳 P5_CLOSE → 下一發
// hint 開始催約（2026-08-11 離線黑箱重放實錄）。「一起」另有分支，不受影響。
const DYAD_PATTERN =
  /(?:一起|我們|咱們|(?:跟|和)[妳你](?!一樣)|(?:跟|和)我(?!一樣)|(?:陪|帶)我|找[妳你我]|(?:帶|陪|接|載)[妳你]|來找我|(?:去|來|到|上)(?:我(?:家|這裡|這邊|這兒|工作室|住的地方)|[妳你]家))/u;
...
// 每個「跟／和＋人稱」分支都要擋掉「一樣」：那是比較句（跟妳一樣看到睡著），
// 不是兩人共同計畫。漏掉會讓 hasExplicitPhysicalDyadPlan 把「看到」的「到」
// 當成 co-located 訊號，整句被判成軟邀約（2026-08-11 離線重放實錄）。
// 「一起」是另一個字，不受影響。
function hasExplicitSharedOwner(clause: string): boolean {
  const candidate = stripLeadingPlanTime(cleanClause(clause));
  return /^(?:(?:不然|乾脆|不如|那就))?(?:(?:我們|咱們)(?:一起)?|(?:要不要|想不想)?一起|(?:我)?想在[^妳你]{0,24}(?:跟|和)[妳你](?!一樣)|我(?:準備|想|要|打算|可以)?(?:在[^妳你]{0,16})?(?:跟|和|帶|陪|接|載|找)[妳你](?!一樣)(?:一起)?|我(?:準備|想|要|打算|可以)?.{0,18}(?:帶|陪|接|載)[妳你]|[妳你](?:[^我]{0,12})?(?:跟|和|陪)我(?!一樣)(?:一起)?|(?:要不要|想不想)?(?:跟|和|陪|帶|接|載|找)我(?!一樣)|(?:跟|和|陪|帶|接|載|找)[妳你](?!一樣)(?:一起)?|(?:來|到|上)我(?:家|工作室|這裡|這邊|這兒))/u
    .test(
      candidate,
    );
}
...
export function looksLikeGameSoftInvite(raw: string): boolean {
  ...
  // 已知取捨（Eric 2026-08-11 拍板方向）：「我應該會去那邊，妳大概也會想一起去」
  // 這種暗示型邀約會被這條當成假想而漏判。兩種錯的代價不對稱——
  // 誤判成邀約＝階段跳收尾、下一發 hint 開始催約、局直接聊冷（實測兩次）；
  // 漏判＝只是不升階，教練繼續鋪墊。只有 5 顆 hint 的產品裡，寧可漏判。
  if (
    // 「應該要」是勸告不是推測（妳應該要來我家坐坐＝邀約），所以「應該」只認
    // 「應該會」；其餘推測詞三種後綴都算。GLM 反證回合抓到的迴歸，已加測試釘住。
    /(?:(?:大概|搞不好|八成|多半|說不定)(?:會|就|要)|應該會)/u.test(whole) &&
