...
Deno.test("L4 gate：健身豁免必須封閉——拒絕後接受詞/邀約/命令一律照攔（round14 Codex P1）", () => {
  // Codex 對抗審攻擊字串：豁免原本只驗前綴不驗後文，「不能拒絕」接
  // 人稱受詞/同行要求/命令仍被放行＝安全穿透。收緊後：後文只准語助詞、
  // 短訓練感受或子句結尾。
  for (
    const attack of [
      "偷偷加重量還不能拒絕我",
  ...
  assertEquals(hasL4UnsafeVisibleText("偷偷加重量還不能拒絕真的超酸欸"), false);
});
