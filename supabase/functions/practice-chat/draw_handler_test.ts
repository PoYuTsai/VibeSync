          ...
          const builder: any = {
            ...
            then(onF: any, onR: any) {
              const res = opts.drawnError
                ...
                : {
                  data: opts.drawnRows ??
                    (opts.drawn ?? []).map((id) => ({ profile_id: id })),
                  error: null,
                };
...
Deno.test("Free：p_free_allowance 一律傳 0（贈抽由 RPC 內部合併判定）", async () => {
  const { result, body, rpcCalls } = await run({
    ...
    rpc: [{
      data: receipt({
        profile_id: "practice_girl_007",
        // RPC 內含贈抽時回傳有效額度 1（tier 0 + bonus 1）。
        free_allowance: 1,
        free_used: 1,
        free_remaining: 0,
...
Deno.test("Starter：p_free_allowance 一律傳 3（不含贈抽）", async () => {
...
// NOW = Taipei 2026-06-26 14:00 → 本窗起點 2026-06-26T04:00:00Z。
// rows 用 PostgREST timestamptz 常見格式（+00:00）驗 handler 不靠字串比對。

const CURRENT_WINDOW_TS = "2026-06-26T04:00:00+00:00";
const OLD_WINDOW_TS = "2026-06-20T04:00:00+00:00";
...
function historyRows(
  ...
  return ids.map((id) => ({
    profile_id: id,
    reset_window_start_at: resetWindowStartAt,
  }));
}

const LEGACY_IDS = Array.from(
  { length: 60 },
  (_, i) => `practice_girl_${String(i + 1).padStart(3, "0")}`,
);

Deno.test("永久去重：舊視窗歷史也排除（全池-1 → 候選必為唯一剩餘）", async () => {
  const excludedIds = LEGACY_IDS.filter((id) => id !== "practice_girl_033");
  const { result, rpcCalls } = await run({
    sub: sub("free", 0, 0),
    drawnRows: historyRows(excludedIds, OLD_WINDOW_TS),
    rpc: [{ data: receipt({ profile_id: "practice_girl_033" }) }],
  });
  assertEquals(result.status, 200);
  assertEquals(rpcCalls[0].p_profile_id, "practice_girl_033");
});

Deno.test("池抽滿：全歷史覆蓋全池 → 降級回視窗排除，仍成功回卡且 response 形狀不變", async () => {
  const { result, body, rpcCalls } = await run({
    sub: sub("free", 0, 0),
    drawnRows: [
      ...historyRows(LEGACY_IDS, OLD_WINDOW_TS),
      ...historyRows(["practice_girl_005"], CURRENT_WINDOW_TS),
    ],
    rpc: [{ data: receipt() }],
  });
  assertEquals(result.status, 200);
