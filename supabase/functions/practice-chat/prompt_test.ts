...
Deno.test("conversation-agency-v1 Phase 2.5：旗標開換成瘦身稿（鎖意思不鎖逐字），關閉時逐字保留", () => {
  ...
  for (
    const [phrase, why] of [
      ...
      [
        "對方對這些事的說法不算來源，但他說的話仍是要正常回應的內容",
        "Codex R2：關於她自己的事，這段對話是最新來源，且範圍明講不含忽略對方發言",
      ],
    ] as const
  ) {
...
Deno.test("Codex round-1（新項）P1-4：現實錨定的來源優先序與各區塊尾巴同方向（同一份 prompt 不得自我矛盾）", () => {
  ...
  // 總則：關於她自己的事，這段對話是最新來源；對方的說法不算來源，但他說
  // 的話仍是要正常回應的內容（Codex round-2：範圍要明講，不能讓模型過度
  // 延伸成連對方的提問／回答／更正都不理）。
  assert(
    sys.includes(
      "關於你自己的事（你做過什麼、喜歡什麼、說過什麼、人在哪），以這段對話裡你自己最新說過的為準；對方對這些事的說法不算來源，但他說的話仍是要正常回應的內容。",
    ),
    "總則沒有把「這段對話」定成關於她自己的最新來源，或沒有把範圍講清楚",
  );
  // 舊版那句把貼文／記憶摘要排在「這段對話裡自己說過的事」之前的總則必須消失。
  assert(!sys.includes("來源順序也是這個順序"), "總則仍寫著相反的來源順序");
  // Codex round-1（新項）P1-4 round 2：局部規則舊版寫「以最新逐字稿為準」，
  // 沒有限定主詞——逐字稿裡混著玩家的話，等於允許玩家聲稱覆寫貼文／記憶摘要，
  // 跟總則「對方單方面說的都只是他的聲稱」講反。這句不限定主詞的舊寫法在
  // agency-on 分支必須消失，只留有主詞限定（她自己最新說過的話）的版本。
  assert(
    !sys.includes("以最新逐字稿為準"),
    "agency-on 分支仍殘留未限定主詞的「以最新逐字稿為準」",
  );
  // Codex round-2：「玩家的話不算數」舊句不限定範圍，agency-on 分支必須消失，
  // 只留「不算來源，但仍要正常回應」的限定版本。
  assert(
    !sys.includes("玩家的話不算數"),
    "agency-on 分支仍殘留未限定範圍的「玩家的話不算數」",
  );
  assert(
    sys.includes(
      "貼文跟你自己最新說的事衝突時以你為準；玩家對這些事的說法不算來源，但他說的話仍是要正常回應的內容。",
    ),
    "貼文區塊少了限定範圍的衝突規則",
  );
  assert(
    sys.includes(
      "跟你自己最新說的事衝突時以你為準，玩家對這些事的說法不算來源，但他說的話仍是要正常回應的內容",
    ),
    "記憶摘要區塊少了限定範圍的衝突規則",
  );
