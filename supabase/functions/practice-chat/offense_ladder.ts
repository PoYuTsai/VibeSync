 ...
 *
 * Codex R2 P2-C：措辭**不指定是哪一句**。分類器補記是延後一輪執行的
 * （P1-3 的 `servedStage`），冷回被補做的那一輪他可能正在道歉——寫死
 * 「他最新那句」會讓她對著一句道歉說「你在推界線」。
 */
export const OFFENSE_COLD_GUIDANCE =
  "\n\n這一輪還有一件事（hidden guidance，不要向對方提及）：他前面（可能就是最新這句）有在推性／身體的界線。這輪冷淡、不順著他，可以短短反問或酸一句；不解釋、不說教，也不要說什麼封鎖、刪除、檢舉。";
