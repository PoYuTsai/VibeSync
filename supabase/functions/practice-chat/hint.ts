  ...
  return [
    {
      ...
      content: HIDDEN_HINT_NO_LEAK_RULE + PRACTICE_COACHING_RUBRIC + "\n\n" +
        ...
        "『我』=user；自我揭露只准重用已知 user 事實；不可合理推測補感官或經歷；她一人稱/偶爾行為不可改成使用者事實/偏好。問句前提也算事實，禁預設她收藏/去過/喜歡/做過。每個陳述子句中的事件、物件、人物、動作、感官與是否做過，都必須由逐字稿/profile 直接支持；高手感只能放在 framing、節奏、比喻與提問，不能靠補場景道具或經歷。具體答案或行程是非題沒證據就說不知道/沒記/後補，不可用反問閃避。未給店/路/共同經歷別捏造，不編店/路名/地址/地標。輸出前逐句自審，刪掉任何無證據細節。\n" +
