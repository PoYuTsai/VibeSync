...
export const HINT_TOOL_SCHEMA: Readonly<Record<string, unknown>> = {
  ...
  properties: {
    ...
    warmUpLabel: {
      type: ["string", "null"],
      description:
        "這顆球的卡片標題，繁中 2-6 字。**兩顆球走不同立場或不同場景時必填**（例「我也喜歡」「約咖啡」）——system 提示裡出現 stanceOptions 就是這種情況。只有兩顆球走同一個立場時才留空。",
      maxLength: 12,
    },
    steadyLabel: {
      type: ["string", "null"],
      description:
        "同 warmUpLabel，對應另一顆球（例「我沒興趣」「約喝一杯」）。warmUpLabel 有填，這欄就一定要填。",
      maxLength: 12,
    },
    coaching: {
      type: "string",
...
- 長短要混，**分則時至少一則壓在 6 字以內**：純情緒短則就是一則，不要怕短（笑死／很強欸／真假／欸不是／蛤／我懂）。真實高手局是「沒有」換行「看過」換行「之前去泰國路邊也一堆在賣」——兩個極短再帶一則有內容的。每則都寫到 14 字還是在寫作文。
- 反問要短得像刀（「妳試過？」「妳酒量如何」「要組隊了嗎」）。
- 繁中口氣照台灣人傳 LINE：常省略主詞（「沒有」「看過」「加了」「剛喝完」）；斷句可以用空格不一定要標點，一則講完不用硬加句號；語助詞挑一個就好（欸／啦／喔／耶／嘛），不要連用、不要用「～」；「哈哈」是節奏不是禮貌，長度就是情緒本身：單獨一則「哈」或「哈哈」讀起來是微句點、在敷衍，自己的回覆**不要**這樣收；真的好笑至少「哈哈哈哈」起跳，或「笑死」接補充。
...
- 台語和注音也混得進去，那是台灣人傳訊的靈魂：注音只當語尾助詞（齁／ㄟ／ㄌ／ㄉ／ㄅ／ㄇ），**笑聲不用注音**——ㄏㄏ／ㄎㄎ／厂厂很屁孩（2026-08-19 Eric 拍板），笑的寫法照上一條的量級。台語寫成諧音字（歹勢、母湯、甘安捏、蝦毀、嘿啊、嘿啦、系金ㄟ、秀秀、阿災）；「蛤」「吼」單字自己一則最台（蛤＝不敢相信，吼＝被逗到或小抱怨）。一則最多摻一個，別整句都是，粗俗的台語髒話不要用；注音要打真的注音符號（ㄟ），不要用長得像的漢字（丂、厂），一律用「妳」稱呼她。
...
- 先讀淺溝通：累→降成本；微測試→先過關；好奇→留懸念；推開→修安全；時間窗→收成；她只回「哈」「哈哈」＝微句點在敷衍→別再追同一個笑點，降成本或換方向。
...
- **立場不猜，改給選項**：只要這句要用到他的喜好、經歷或立場，而逐字稿裡他沒說過，就**不要替他決定**——讓兩顆球各走一個立場（一顆「我也愛」一顆「沒興趣」；一顆咖啡一顆喝一杯）。猜錯會讓他下一句就露餡，給選項永遠比猜安全。該給選項的時候系統會另外告訴你，連標籤怎麼填一起講。
...
function stanceOptionPrompt(turns: PracticeTurn[]): string {
  ...
  return `stanceOptions: 她這句在問你。如果問到的是**他沒表態過的喜好或經驗**，絕對不要替他決定——兩顆球各走一個立場，warmUpLabel／steadyLabel 必填（例：她問「那你衝浪嗎」→ 一顆標〔我也想試〕「沒衝過\u3000但看起來蠻好玩」；一顆標〔沒興趣〕「沒欸\u3000對浪沒什麼興趣」）。兩顆都替他選同一個立場＝失敗。**標籤和立場是綁一起的**：只要兩顆球走了不同立場，兩個 label 就一定要填——沒填標籤，使用者在卡片上只會看到「升溫回覆／穩住回覆」，根本看不出這是二選一，等於你這輪白給了選項。逐字稿裡他已經表態過，就照他講過的；這句根本沒有喜好可選（只是問你狀態），或她其實是在測你、嗆你、懷疑你的動機，就不要給立場——照接住測試的打法走，別認真解釋自己。\n`;
}
