...
>([
  ["standard／有 mapping（001）／旗標關", {
    messages:
      "a7e60b1b38da57ee2de777149c5ab0d9ac896e45812026b9215547709547ab42",
    response:
  ...
  ["beginner／有 mapping（001）／client seed／旗標關", {
    messages:
      "a37b1957c414989f3092f7da83735479b58dd49ae9af5e2b0fd88866363c9d01",
    response:
...
>([
  ["standard／片段／style 關", {
    messages:
      "7dc284e5b0487e7d2bc6747870bb99f743b98d292896b2985f13c5a0f733cc55",
    response:
  ...
  ["beginner／片段／style 關／thread", {
    messages:
      "ff13ebe0dbdcacdccbf84a9af3db87944b77093937cad7446e9a0d6fcf7b4990",
    response:
  ...
  ["beginner／片段／style 開／thread", {
    messages:
      "6a9eecb97812a0ea1fd4204ad24d38fed21bae14a5cd3f702651e37a8227ba88",
    response:
      "409f1af7e9d5f2f155f664cefacea35fe93780eab5c259be2394b648d4ac8364",
    recentFacts:
  ...
  ["standard／片段／style 開", {
    messages:
      "07e4a809b3cd87be2e2aeaeb960c738c88993eefc65516f46f8ce6ec2e9b3bed",
    response:
      "444e4e27dafce2e0ec8c13b924c4b46dd05d51f72ef1d863c13a22a5ce6f318b",
    recentFacts: "none",
    calls: 1,
...
Deno.test("Phase 3.3 旋鈕 off／agency off：回覆逐字不動，telemetry 連欄位都不多一個", async () => {
  ...
  // 認不得的值 fail-closed 成 off（2026-09-04 刪掉的 `prompt` 臂也走這條）。
  const unknownArm = await shapeExperimentRun({
    PRACTICE_CONVERSATIONAL_AGENCY_ENABLED: "true",
    PRACTICE_AGENCY_SHAPE_EXPERIMENT: "prompt",
  }, SHAPE_ACCOMMODATING_REPLY);
  assertEquals(unknownArm.json.reply, SHAPE_ACCOMMODATING_REPLY);
  assert(!("shapeTruncatedBubbles" in unknownArm.agency));
