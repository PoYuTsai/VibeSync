...
// ── 永久去重：hasEligibleDrawCandidate（handler 據此決定是否降級回視窗排除）──
Deno.test("永久去重：排除全池-1 → 仍有候選，且 select 必回唯一剩餘", () => {
  const allButLast = new Set(
    GIRL_PROFILES.slice(0, LEGACY_CATALOG_SIZE - 1).map((g) => g.profileId),
  );
  assert(hasEligibleDrawCandidate({ excludedProfileIds: allButLast }));
  const only = selectPracticeDrawProfile({
    excludedProfileIds: allButLast,
    seed: "perm-only-1",
  });
  assertEquals(girlIndex(only.profileId), LEGACY_CATALOG_SIZE);
});

Deno.test("永久去重：排除全池 → 無候選（池抽滿，觸發 handler 降級）", () => {
  const all = new Set(
    GIRL_PROFILES.slice(0, LEGACY_CATALOG_SIZE).map((g) => g.profileId),
  );
  assertEquals(hasEligibleDrawCandidate({ excludedProfileIds: all }), false);
});

Deno.test("永久去重：currentProfileId 也算排除（唯一剩餘＝current → 無候選）", () => {
  const allButLast = new Set(
    GIRL_PROFILES.slice(0, LEGACY_CATALOG_SIZE - 1).map((g) => g.profileId),
  );
  const lastId = GIRL_PROFILES[LEGACY_CATALOG_SIZE - 1].profileId;
  assertEquals(
    hasEligibleDrawCandidate({
...
Deno.test("永久去重：eligible 判斷與 catalogSize 切池取交集（排除前 60、宣告 100 → 有候選）", () => {
