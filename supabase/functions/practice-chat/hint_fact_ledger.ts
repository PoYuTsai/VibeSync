...
// 中午有空——「可能」是推測副詞不是可用性）。
// 「忙」加負向 lookbehind：擋「幫忙」詞中匹配（round13 gd5：「還分配你
// 『幫忙把關殺價』」被跨 gap 抓成 busy 述語，湊出捏造的 busy_at claim）；
// 真忙碌（很忙/在忙/週六忙）前一字不會是「幫」。
const SCHEDULE_STATUS =
  "沒空|不方便|已經有約|有約|有空|可以|(?<!可)能(?![量力耐])|沒事|方便|排得開|休假|下班|(?<!幫)忙|有事|有安排|排滿|要開會|要上班|要上課|要看醫生|值班|出差|在公司|在學校|在家裡|在外面";
const HISTORY_TIME =
...
function polarityAt(
  ...
  // 「沒有把/沒有將」是把-字句否定（round13 gd5：「沒有把她釋出的時間線索
  // （下午可以）轉成確認」）：否定的是處置動詞（轉成），不是內嵌 claim 的
  // 述語，不得回頭翻內嵌極性。
  return /(?:不是|並非|沒有(?![把將])|沒在|不同(?:年|歲|名|校|業|鄉|城)|不住|不叫|不念|不讀|不養|不喜歡|不愛|不當|不做)/u
      .test(window)
    ? "negative"
    : "positive";
}
...
const NON_PLACE_COMPOUND_TAIL =
  /(?:退路|套路|思路|出路|心路|後路|活路|絕路|末路|門路|歪路|岔路|網路|走路|迷路|問路|報路|認路|指路|記路|記得路|不記得路|忘記路|沒記路|知道路|上路|帶路|鋪路|領路|網站|誤區|雷區|盲區|禁區|舒適區|安全區|城市|都市|超市|冰山|靠山|火山|爬山|下山|上山)$/u;
...
export function extractHintFactClaims(
  ...
  const thirdPartyNamePatterns: Array<{
    ...
    },
    {
      ...
      // 用 lookahead 擋（丟出小測試、傳統、送到…）；「丟球」是丟球
      // 考驗的比喻（round10 gd2「丟球考驗你」）不是丟給某人。
      pattern:
      // round13 gd6：「叫外送比較多」——「外送」的「送」詞中匹配把
      // 「比較多」抓成人名；送收動詞前一字是「外」（外送/外賣配送語境）
      // 就不是把東西送給某人。
        /(?:傳(?!給|統|說|來|開|訊|話|達|遞|真|承|神|聞|奇)|(?<!外)送(?!給|到|出|來|回|上|貨|禮)|丟(?!給|出|回|下|上|的|了|掉|臉|包|進|在|球))\s*([\p{Script=Han}A-Za-z·・]{2,20})(?=[，,。！？!?；;\s]|$)/gu,
      valueIndex: 1,
      introduction: false,
    },
    {
  ...
  // round13 gd2：「路」的 kill-list 補「來」（一路來回＝副詞，同族
  // 一路追問/一路聊）。
  const namedPlacePatterns = [
    /(?:^|[\s，,。！？!?；;：:\p{S}])(?:在|去|到|位於|路過|靠近|就在)?\s*([\p{Script=Han}0-9]{2,40}(?:路|街|大道)[\p{Script=Han}0-9]{0,20}(?:號|樓))/gu,
    /(?:^|[\s，,。！？!?；;：:\p{S}])(?:在|去|到|位於|路過|靠近|就在)?\s*([\p{Script=Han}A-Za-z0-9·・]{1,24}(?:站(?![著起穩])|路(?![人痴燈線過問聊順講談延帶追來])|街(?![道頭])|巷(?![口弄])|區(?![域分別隔])|市(?![集場面況])|縣|鄉(?![下愁])|鎮(?!定)|村|里(?![面頭程])|町|山(?![頂腰腳])|公園|夜市|商圈|碼頭|廣場|大樓|中心|101))/gu,
    /(?:在|去|到|位於|路過|靠近|說出|說|回答|地點|位置|地址)\s*([\p{Script=Han}A-Za-z0-9·・]{1,24}(?:站(?![著起穩])|路(?![人痴燈線過問聊順講談延帶追來])|街(?![道頭])|巷(?![口弄])|區(?![域分別隔])|市(?![集場面況])|縣|鄉(?![下愁])|鎮(?!定)|村|里(?![面頭程])|町|山(?![頂腰腳])|公園|夜市|商圈|碼頭|廣場|大樓|中心|101))/gu,
  ];
  for (const pattern of namedPlacePatterns) {
    for (const match of text.matchAll(pattern)) {
      ...
      if (
        ...
        // round10 gh5「不要一路追問」：副詞「一路」非路名——裸「一路」或
        // 前一字是虛詞/否定詞時剔除；真路名（民生一路）前一字是專名構詞。
        /(?:^|不|別|就|還|都|但|才|又|再|要|直)一路$/u.test(anchor) ||
        // round11 gh5「還沒仔細記那區怎麼走」：指示詞＋字尾（那區/這站/
        // 那條街）是回指她問過的地點，不是新地名主張；真地名的字尾前一字
        // 是專名構詞（信義區/民生社區），絕不會是那/這。
        /(?:那|這)(?:一)?(?:條|個)?(?:區|站|街|帶)$/u.test(anchor) ||
        // round12 gh5「速約任務：不報地區」：否定動詞＋通用地點名詞＝守規
        // 宣言不是地點主張；帶真地名的否定（不報南京復興區）尾詞非通用
        // 名詞，不豁免照抓。
        /^(?:先|暫|絕|都|就|還|也|再)?(?:不|別|沒|未|勿|免)(?:用|必|會|要|能|想|得|再|先)?(?:直接|真的|馬上|急著)?(?:報|說|提|講|給|透露|提供|洩漏|公開|曝光)(?:出|到)?(?:任何|具體)?(?:地區|市區|區域|區|地址|地點|位置)$/u
          .test(anchor) ||
        // round13 bh5「深蹲之路辛苦了」：X之路＝歷程比喻（減肥之路/重訓
        // 之路），不是路名；真路名（民生東路）尾字前不會是「之」。
        /之路$/u.test(anchor) ||
  ...
  for (
    const match of text.matchAll(
      ...
      // round12 gh5：尾端 (?!的) 會被回溯繞過（「是聽人推薦的」切成「聽人推」
      // 當店名）——改要求店名頂到非漢字邊界，堵死切詞；真店名貪婪捕獲本來
      // 就頂到標點/引號，不受影響。
      // round13 gh6：「私藏好店…但我是那種只給…的人」——「是」後接指示
      // 量詞「那種/這種」＝自我歸類句不是店名；真店名不會以那種/這種開頭。
      /(?:店|店家|咖啡店|餐廳|酒吧|那間|這間)(?![」』”"])[^。！？!?；;]{0,8}?(?:(?<![只還就但可也算而不])是|叫|名為|稱作)\s*[「『“"]?(?!什麼|哪|誰|怎|有|在|那種|這種)([^」』”"，,。！？!?；;、的妳你我]{2,30})(?!\p{Script=Han})/gu,
    )
  ) {
    add({
      owner: "world",
      ...
      for (const match of text.matchAll(pattern)) {
        ...
        if (
          ...
          /(?:不知道|不記得|忘了|沒記住|先確認|查一下|問一下|還沒|沒有|沒去過|保密|賣關子|賣個關子|勉強|再說|再告訴)/u
            .test(anchor) ||
          // round12 gh5 #10：「說實話那間…」句首語用標記撞上「X＋那間/那家」
          // 店名形＝誤殺；語用標記不是店名。
          /^(?:說實話|說真的|老實說|老實講|坦白說|坦白講|講真的|講白了|說白了|不瞞妳說|不瞞你說|其實)$/u
            .test(anchor) ||
