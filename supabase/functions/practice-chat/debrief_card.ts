...
function clauseClaimsPartnerInitiatedInvite(value: string): boolean {
  ...
  // 「才能/才會…見面」＝門檻條件句：她還沒邀約，是在講需要什麼條件才會
  // 考慮（「她需要更多安定感才能考慮見面」，2026-07-24 Mabel FP）。
  if (/(?:才能|才會|才可能|才願意).{0,10}(?:見面|碰面|邀約|約|邀)/u.test(compact)) {
    return false;
  }
...
function assertHintAssessment(opts: {
  ...
  // Codex 審查反例），與明示「提示錯」同視為翻案。
  const visiblyReversesHint =
    /(?:提示|建議)(?:(?:本身|內容|那句|其實|真的|確實|有點|完全|根本|實在|太|很|偏|是)){0,3}(?:錯|不對|不該|太急|偏保守|無效|不好|不合適|不適合|有問題|失準|誤判)|(?:比起|與其)(?:照|依|按|用|貼)?(?:提示|建議)/u
