...
// ADDRESSEE_PLAN_CUE 的 [妳你]下來 接手，不漏。
const GENERIC_PROPOSAL =
  /(?:(?<![好難耐])(?<!繼續)(?:看|吃|喝|玩)(?!(?:起來|下來))|(?<!換)(?<!(?<!一)起)(?<!下)來|(?<![被挨])打(?!(?:算|工|字|掃|卡|招呼|氣|擊))|(?<!下)去|逛|走(?!路)|爬|唱|約|碰面|見面)(?!(?:下去|對了|錯了))[^，,。！？!?；;敢]{0,18}(?<!了)(?:吧|嗎|好不好|怎麼樣|如何)(?:[?？])?$/u;
const EXECUTION_CUE =
...
const INTENT_QUESTION_CLAUSE =
  /(?:還會想?再?|會想?再|還?敢再?)(?:被[^我，,。！？!?；;]{0,3})?(?:去|爬|來|吃|喝|看|逛|玩|走|打|唱)[^，,。！？!?；;我]{0,10}嗎/gu;
