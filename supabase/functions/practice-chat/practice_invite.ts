...
const CONCRETE_TIME =
  /(?:明天|後天|今晚|明晚|這週|週末|週[一二三四五六日天]|禮拜[一二三四五六日天]|下班後?|中午|下午|晚上|[0-9０-９一二三四五六七八九十兩]{1,3}[點時])/u;
...
const ADDRESSEE_PLAN_CUE =
  /(?:我(?:想|要|會|可以)?(?:去|來)?(?:接|載|帶|找|約|請)[妳你]|(?:接|載|帶|找|約|請)[妳你](?:去|來|吃|喝|看|玩|走)?|(?:在|到).{0,12}等[妳你]|[妳你](?:(?:直接)?(?:過來|過去)|直接(?:來|到)|下樓|出來|出門)|我(?:到|到了).{0,4}(?:叫|跟)[妳你](?:說)?|(?:跟|和)[妳你].{0,10}(?:去|來|吃|喝|看|逛|玩|走|爬|打|唱|見|碰面)|(?:[妳你].{0,10}|(?:明天|後天|今晚|明晚|週末|週[一二三四五六日天]|禮拜[一二三四五六日天]).{0,6})(?:留|空出|撥)(?:半小時|一小時|[0-9０-９一二三四五六七八九十兩]{1,3}分鐘|時間)?給我)/u;
...
// 這類複合詞的尾字不得當提案動詞；語尾段落再排除「敢」擋住反問句
// （「還敢跟嗎」是追問對方敢不敢，不是提案）。
const GENERIC_PROPOSAL =
  /(?:(?<![好難耐])(?:看|吃|喝|玩)|(?<!換)(?<!(?<!一)起)來|(?<![被挨])打|去|逛|走|爬|唱|約|碰面|見面)[^，,。！？!?；;敢]{0,18}(?:吧|嗎|好不好|怎麼樣|如何)(?:[?？])?$/u;
const EXECUTION_CUE =
