...
// round14 gh4 補：閉引號也是子句邊界（「…跟你多聊一點」聽起來——引用她
// 原話後的「聽起來」的來不得被湊成同行動詞）。
// 跟妳說的是說一聲，「看」在下一子句，跨逗號湊對＝假邀約）。
const ADDRESSEE_PLAN_CUE =
  /(?:我(?:想|要|會|可以)?(?:去|來)?(?:接|載|帶|找|約|請)[妳你]|(?:接|載|帶|找|約|請)[妳你](?:去|來|吃|喝|看|玩|走)?|(?:在|到).{0,12}等[妳你](?![的回])|[妳你](?:(?:直接)?(?:過來|過去)|直接(?:來|到)|下樓|下來|出來|出門)|我(?:到|到了).{0,4}(?:叫|跟)[妳你](?:說)?|(?:跟|和)[妳你][^，,。！？!?；;」』”'"]{0,10}(?:去|來|吃|喝|看|逛|玩|走|爬|打(?!(?:個|一)?(?:算|工|字|掃|卡|招呼|氣|擊|分))|唱|見|碰面)|(?:[妳你].{0,10}|(?:明天|後天|今晚|明晚|週末|週[一二三四五六日天]|禮拜[一二三四五六日天]).{0,6})(?:留|空出|撥)(?:半小時|一小時|[0-9０-９一二三四五六七八九十兩]{1,3}分鐘|時間)?給我)/u;
// 中文沒有詞邊界：「看【來我這】關過了」「原【來我這】邊也有」都會誤命中
// 「來我這（裡）」。看來／原來／後來／本來／從來／向來／近來／說來／算來
// 是副詞，不是「來我這裡」。負向後顧只排這幾個無歧義的字——「想來我家」
// 「過來我這」「有空來我這」都必須照樣判得出來。
const IMPLICIT_ARRIVAL_CUE =
  /(?<![看原後本從向近說算])(?:(?:來|過來)(?:找我|我家|我這邊|我這)|(?:到|來)我(?:家|這邊|這))/u;
...
// 問她的品味感受或推測差異，不是把她放進行動計畫。round13 gh6 二發補
// 「堅持」入同族（妳喝拿鐵有什麼堅持嗎）。round14 gh1/gh6 通則化：語尾
// 視窗內出現「什麼/哪種/那種」＝歸類/屬性問句（是慢慢咀嚼那種嗎、走什麼
// 風格嗎、那種舌頭嗎），整族不是提案；真提案報的是具體物件（喝一杯/
// 那間店），不用疑問代詞。
const GENERIC_PROPOSAL =
  /(?:(?<![好難耐])(?<!繼續)(?:看|吃|喝|玩)(?!(?:起來|下來|到))|(?<!換)(?<!(?<!一)起)(?<!下)來|(?<![被挨])打(?!(?:算|工|字|掃|卡|招呼|氣|擊|分))|(?<!下)去|逛|走(?!路)|爬|唱|約|碰面|見面)(?!(?:下去|對了|錯了))(?:(?!什麼|哪種|那種)[^，,。！？!?；;敢]){0,18}(?<!了)(?<!一樣)(?<!挑剔)(?<!講究)(?<!堅持)(?<!偏好)(?<!習慣)(?<!的人)(?<!類型)(?:吧|嗎|好不好|怎麼樣|如何)(?:[?？])?$/u;
const EXECUTION_CUE =
