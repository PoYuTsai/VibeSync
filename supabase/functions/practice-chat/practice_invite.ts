...
const GENERIC_PROPOSAL =
  /(?:(?<![好難耐])(?<!繼續)(?:看|吃|喝|玩)(?!(?:起來|下來|到))|(?<!換)(?<!(?<!一)起)(?<!下)來|(?<![被挨])打(?!(?:算|工|字|掃|卡|招呼|氣|擊|分))|(?<!下)去|逛|走(?!路)|爬|唱|約|碰面|見面)(?!(?:下去|對了|錯了))[^，,。！？!?；;敢]{0,18}(?<!了)(?<!一樣)(?<!挑剔)(?<!講究)(?<!偏好)(?<!習慣)(?:吧|嗎|好不好|怎麼樣|如何)(?:[?？])?$/u;
const EXECUTION_CUE =
...
const INTENT_QUESTION_CLAUSE =
  /(?:還會想?再?|會想?再|還?敢再?|會有想|有沒有(?:很)?想|會不會想)(?:被[^我，,。！？!?；;]{0,3})?[^，,。！？!?；;我]{0,3}?(?:去|爬|來|吃|喝|看|逛|玩|走|打|唱)[^，,。！？!?；;我]{0,10}嗎/gu;
...
// 承諾，不是把她放進見面計畫——整個子句剝除，免得子句裡的改天/下次把
// 後面的品味問句拉成 soft 邀約。「帶妳去」沒有分享動詞，不受影響。
const SHARE_CONTENT_CLAUSE =
  /[^，,。！？!?；;]*(?:整理|列|傳|發|寄|截圖|分享)[^，,。！？!?；;]*給[妳你][^，,。！？!?；;]*/gu;
// 真機 gh7 補（2026-07-23）：「會偷偷想像自己去哪嗎」＝問她的想像/夢想清單，
// 不是把她放進行程——想像/幻想後的子句剝除。「別只想像了，直接一起去」的
// 真提案在下一子句，不受影響。
const IMAGINATION_CLAUSE = /(?:想像|幻想|做夢夢到)[^，,。！？!?；;]*/gu;
...
export function practiceInviteLevelFor(value: string): PracticeInviteLevel {
  ...
  const positiveText = compact.replace(NEGATED_PLAN_CLAUSE, "").replace(
    ...
    "",
  ).replace(INTENT_QUESTION_CLAUSE, "").replace(
