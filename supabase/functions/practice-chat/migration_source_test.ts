...
// ── 訪客模式移除：prepare RPC 還原無條件歸零 ──────────────────────────────
const removeGuestSkipMigration = await Deno.readTextFile(
  new URL(
    "../../migrations/20260801150000_practice_prepare_remove_guest_skip.sql",
    import.meta.url,
  ),
);

Deno.test("prepare_practice_subscription_usage 還原無條件歸零、簽名與鎖不變", () => {
  assert(
    removeGuestSkipMigration.includes(
      "CREATE OR REPLACE FUNCTION public.prepare_practice_subscription_usage",
    ),
  );
  // 匿名分支必須整段拿掉
  assert(!removeGuestSkipMigration.includes("v_is_anonymous"));
  assert(!removeGuestSkipMigration.includes("auth.users"));
  // row lock 與 fail-closed 語意保留
  assert(removeGuestSkipMigration.includes("FOR UPDATE"));
  assert(
    removeGuestSkipMigration.includes("PRACTICE_SUBSCRIPTION_NOT_FOUND"),
  );
  // 簽名零改動（不得出現第二參數）
