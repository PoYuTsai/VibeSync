...
最高優先漏網例（另有對應直接證據則保留；否則即使自然、合理或玩笑也必修）：user 說追劇到兩點，Hint 的「你追什麼劇」把 user 事實轉給她；「靠意志力撐到最後」也無證據。user 只說路過聞香、她只問哪家時，安全句是「叫{店名}，我路過時聞到很香」；{路名}、只記得香味、咖啡不懂、很想進去、停下/查名/進店/「路過聞到香就記住了」都無證據，coaching 教「填不出就說只記得香味」也必修。她玩笑「怕被你拿去裝懂」不是 user 答案；「裝懂我倒不至於」改 {真實回應} 或直接接她已說內容。「我有感/香會讓人停下來」無 user 感受證據，用 {真實感受}。非答句「我最近喝到…意外」也屬 user 自揭，無 user/trusted 直證即刪。她的現況只認 assistant_turn。
...
逐句拆最小命題；過去／現在須同承諾者完整直證，被評者非 owner（「你鼻子太靈」≠user 自認鼻子靈）；無據即修。單次事件／單一物件只證該次／該物件，不證習慣、類型、頻率、數量、傾向或因果（一次早睡≠早睡派；存一家店≠收藏很多；追到兩點≠一開就停不下來）。修正只刪問題子句或換原子槽，不另造事實；純未來提議與無前提反問可保留。泛評/認同她評user/非答句「我最近喝到…意外」皆屬user立場/經歷，無同 owner 直證即刪；忠實改述她可留。
