...
export function buildGroundingReviewMessages(opts: {
  ...
  const continuity = opts.surface === "debrief"
    ? "已套用 Hint 是 server 鎖定的正確決策；只改已確認有問題的最小子句，未涉案欄位逐字保留；Game 若只修 suggestedLine 僅同步 nextFirstLine，不得改 Hint 接球點、策略或邀約路線。"
  ...
  const verificationSignal = opts.verificationPass
    ? "這是獨立第二道對抗複核。假設前一位審查者漏掉了一個很自然的第一人稱幻覺或主客體誤讀，不能因為上一版已被審過就信任它；重新從逐字稿取證，尤其找『沒記住／不知道／有點餓／某種香氣』這類無證據答案。"
    : "";
  ...
  return [
    ...
    { role: "assistant", content: opts.previousCandidate },
    {
      ...
      content:
        ...
        "輸出前逐欄盤點所有以 user 為主語的主張（Debrief 分析欄的『你』、貼句的『我』都算）；每項須指向逐字稿 user turn 或明示 server-trusted user evidence。另盤點誰問、誰說、誰邀約、誰做了什麼，並對齊同角色逐字稿；不能因語氣自然就略過。" +
        "不得把『她問你有沒有去／喝／看』改寫成她邀你去，也不得把使用者自己的提議改寫成對方主動。" +
        "上一版候選不是證據；assistant 對 user 的問句、玩笑、吐槽、猜測、肯定式推測或摘要，只證明她說過，不證明內容為真。若使用者尚未親自回答或陳述，就不能替使用者補答案。" +
        "『沒有證據』也不等於否定或忘記：沒記住、不知道、沒去過、不是、有點餓、想保密、看到第幾集、聞到堅果味等記憶、狀態、感官、數量、原因與意圖，同樣都要有使用者證據；也不得把對方的猜測改寫成使用者事實。" +
        "『被抓包／妳說中了／確實／就是／對啊』若在承認對方猜測，整段被承認內容都成了 user 事實；沒有 user turn 或 server-trusted user evidence 就必須移除或留變數。已知結果也不支持補原本計畫、意圖、頻率或因果：『追劇到兩點』不支持『本來只看一集』，『路過店』不支持『永遠沒下次／口袋名單存很多』。" +
        "問句、假設、條件句、泛稱人物、把問題交回對方、主觀感受或未來提案，不等於宣稱既成事實。" +
        "但任何具名人物、地點、時間、偏好、經歷、關係、行程或聯絡資料，只要上方沒有直接證據，就必須刪除或改成不預設答案的自然說法。" +
        "如果缺的是對方正在追問的答案，把缺失子句改成明確可替換欄位，例如『我在追《{劇名}》』、『是{店名}，我{有／沒有}進去喝』或『我當時{餓／不餓}』，讓使用者填真值；不能為了讓句子好貼而編答案，也不能自行聲稱不知道、沒記、保密或稍後補。" +
