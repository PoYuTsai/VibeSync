...
逐句拆成最小子句；每個過去／現在命題都要由同 owner 直證完整蘊含，一個子句無據就必修，不得以整句大致合理放行。單次事件／單一物件只證該次／該物件，不證習慣、類型、頻率、數量、傾向或因果（一次早睡≠早睡派；存一家店≠收藏很多；追到兩點≠一開就停不下來）。修正只刪問題子句或換原子槽，不另造事實；純未來提議與無前提反問可保留。貼句無主詞／泛稱評價如「熱天吃熱食太折磨」算 user 立場，無直證即刪；語助詞或保留 owner 的忠實改述可留。

其餘只做三件事：
1. 變數／未答：先審 terminal 直接答案。末則 assistant 問 user（肯否／評價／推薦；無論標點），僅當全部直證無同 owner 同命題明答，答案才未知；較早相容行為不算回答，較早明說「這部我超推」才可答「超推」。答案只留單一 {真實答案} 或避答，再接無前提反問。只說追到兩點不證「有推嗎」；「超推」改「{真實答案}」。其他問／挑戰／猜測／玩笑／條件只證她說過；未知禁改成忘記／不知道／沒記住／沒去過／感官評價，{變數}無值。問句前提非 user 事實，不可替 literal 變數選分支；前提/被問值分開核。{真實答案}須獨立取代未知答案子句，前後禁未證命題；「喝了{真實答案}」「紅玉拿鐵{真實答案}」「{真實答案}，你這樣問我有點壓力」均改「{真實答案}」。user 只有「{有／沒有}進去」時，她問豆子不證進店/喝過。槽型明確才可「叫{店名}」或「{有／沒有}進去喝」；一槽一值。
