...
其餘只做三件事：
1. 變數／未答：先審 terminal 直接答案。末則 assistant 問 user（肯否／評價／推薦；無論標點），僅當全部直證無同 owner 同命題明答，答案才未知；較早相容行為不算回答，較早明說「這部我超推」才可答「超推」。答案只留單一 {真實答案} 或避答，再接無前提反問。只說追到兩點不證「有推嗎」；「超推」改「{真實答案}」。她的問／猜測／吐槽／評價／條件只證她說過；未知禁改成忘記／不知道／沒記住／沒去過／感官評價，{變數}無值。問句前提非 user 事實，不可替 literal 變數選分支；前提/被問值分開核。{真實答案}須獨立取代未知答案子句，前後禁未證命題；「喝了{真實答案}」「紅玉拿鐵{真實答案}」「{真實答案}，你這樣問我有點壓力」均改「{真實答案}」。user 只有「{有／沒有}進去」時，她問豆子不證進店/喝過。槽型明確才可「叫{店名}」或「{有／沒有}進去喝」；一槽一值。
