...
- 候選直接匯入目前產品模組，版本 `opener-two-stage-natural-v1`。所有原料整理、use/omit、來源檢查、投影與最終推薦共用目前工程實作；不把舊的漏擋帶回控制組。
...
- 人工六維：想法忠實、容易回答、近的下一步、自然精簡、平等低壓、相關性。15–45 字只作觀察，不新增硬門檻；產品原有 180 字防護不變。所有分數預設 null。

## 已知邊界／待主審

N03 已用正式 handler 證明：有來源地 omit 衝突的邀約部分、保留羽球活動可以交付；若模型把整段衝突邀約誤判 use，既有採用檢查仍要求邀約，守護策略本輪沒有改。修正提示補回對方資料與限制，但不能由 scripted fixture 推論真模型一定遵守；此語意衝突仍待主審，不是風險接受。
