...
**兩列 check_out 是不同的問題**（Eric 2026-09-05 裁決，刻意如此）：
`checkOutStructuralFail` 的分母是 agency 輪數＝**無條件**的整體失敗率；
「改寫注入後仍失敗」的分母是**注入輪數**＝**條件機率**，量的是 4.6 刀 2 那道
改寫指令注入之後還救不回來的比例，那才是刀 2 的成效指標。兩列並列看。
