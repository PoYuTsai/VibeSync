...
// ── 指標的雜訊帶（2026-08-20 校準）────────────────────────────
// 同一份 prompt、同一份自介、n=40 連跑三批（grace）／兩批（Ally）量到的擺盪。
// 改動要能被相信，效果必須明顯大於這裡的數字，否則就是在讀雜訊。
//
//   指標                grace          Ally      擺盪
//   複述（judge）       14/14/15       10/9      ±1   ← 最準
//   實質問句（正則）    13/12/12       15/14     ±1   ← 最準
//   隨口（judge）       15/15/12       21/19     ±3
//   轉折骨架（正則）    29/31/31       27/22     ±5
//   同骨架（judge）     22/21/25       13/19     ±6
//   唸稿開頭（正則）    19/13/17       20/25     ±6
//
// 兩件事跟直覺相反：(1) 準不準取決於指標本身，不是「正則 vs LLM」——最緊的
// 複述是 LLM 判的，最鬆的唸稿開頭是正則。(2) 骨架類指標在 n=40 全部落在
// ±5 以上，2026-08-19 那五次「27→24／27→20 有降」的讀數整批在雜訊帶內。
// 要對骨架下結論得把 n 拉到 120 以上，或換一個更緊的量法。
//
// ── 客觀指標 ────────────────────────────────────────────────
/**
