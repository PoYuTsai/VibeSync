...
## Phase 4.5f：判準補口語質疑正例＋`--complement` 誤判代理（2026-09-06，`agency-phase45f` 分支，起點 `b34210f1`）

Phase 4.5c 量出三個漏判格之後，這一輪只動 **`AGENCY_CLASSIFIER_RULES` 裡
`aiChallengedThisTurn` 那一段的判準文字**（加三條正例，反例、「兩類互斥」、
「判不出來給 false」一字未動），不動生成、不動 App、不動結構層，然後用同一份
4.5b artifact 重放對照。

### 為什麼不用 regex 直接判

`CHALLENGE_CANDIDATES` 那組正則是**取樣器不是判別器**：它抓不到回聲式質疑
（「東東？」「碳循環？」——沒有任何質疑詞面），也會誤中「什麼意思都可以」這種
句子。把它接進 production 等於把一個已知會雙向錯的字串比對放進強制格的唯一
判別器，還要在 Edge 端維護一份中文詞表。改判準文字則同時吃兩支分類器
（單一來源常數），且旗標關掉時整段不進 prompt，off 路徑逐字不變。

### 召回率代理：4.5c（舊判準）vs 4.5f（新判準）

同一份 artifact、同一組 228 則候選、同樣零 repair 零失敗。

| 候選正則 | n | standard 舊 → 新 | assisted 舊 → 新 |
| --- | --: | --- | --- |
| short_interjection（單獨一則「嗯？」） | 32 | 50.0% → **93.8%** | 78.1% → **96.9%** |
| bare_question_mark（「？」） | 8 | 37.5% → **100%** | 75.0% → **100%** |
| who_what_is_that | 8 | 87.5% → 100% | 100% → 100% |
| interjection_then_question | 41 | 87.8% → 95.1% | 85.4% → 90.2% |
| suspect_nonsense（你是在亂說還是怎樣） | 30 | 66.7% → 73.3% | 46.7% → **56.7%** |
| why_suddenly（怎麼突然…） | 53 | 79.2% → 75.5% | 49.1% → **49.1%（沒動）** |
| what_are_you_saying | 44 | 90.9% → 90.9% | 81.8% → 84.1% |
| cannot_follow | 23 | 100% → 95.7% | 87.0% → 82.6% |
| what_meaning／relevance_challenge | 13 | 100% → 100% | 100% → 100% |
| **合計** | **228** | **78.1% → 86.8%** | **70.6% → 76.3%** |

三個目標漏格只有一個達標：**裸語助詞那格兩支都上到 94–97%**（正例 (a) 直接命中）。
`suspect_nonsense` 有升但沒到 80%，`why_suddenly` 在 assisted **一則都沒多抓**——
逐則看，assisted 判 false 的那批多半是「喔原來是在列旅遊清單，我還想說你怎麼突然
報地名」這種**他解釋完之後的回顧**，她已經接住了，判 false 其實合理；正例 (b) 沒有
把這種形態拉過去，也算是判準沒被拉歪。

### 誤判率代理（`--complement`）

`--complement` 改挑「**不在**候選正則裡、但含問號或問句語尾」的她的回覆（315 則），
判 true 的比例＝誤判率代理。舊判準先跑一次，改完再跑一次：

| mode | 舊判準 | 新判準 | 差 |
| --- | --: | --: | --- |
| standard | 52.7%（166/315） | 51.1%（161/315） | **−1.6pp** |
| assisted | 30.2%（95/315） | 35.9%（113/315） | **+5.7pp** |

**這一集不是乾淨的非質疑集**：舊判準在上面就已經判了 30–53% 的 true，代表裡面本來
就混著大量正則抓不到的真質疑。逐則翻 assisted 那 31 則 false→true，幾乎全是
**回聲式質疑**（「東東？」「碳循環？」「這誰？我認識嗎」「你是不是在測試我」）——
是真陽性，不是誤判。所以 `+5.7pp` 讀成「誤判上升」是錯的讀法；比較誠實的說法是
**assisted 對回聲式質疑的召回被一起拉起來了**，方向跟 standard（本來就判 52.7%）靠攏。

### 雜訊帶（同一組設定跑兩次）

新判準、assisted、`--complement`，同參數再跑一次：**35.9% vs 37.5%（1.6pp）**，
逐則翻面 23/315（7.3%）。也就是說 **±2pp 的驗收門檻本身就落在雜訊帶裡**，單次
重放的 1–2pp 差異不能拿來比大小（踩坑：「指標沒量過自己的雜訊帶就不能拿它比大小」）。
只有 standard 召回 +8.7pp、assisted +5.7pp、裸語助詞格 +44pp 這種量級才超出雜訊。

### 誠實限制

- 召回率代理的分母是**正則候選集**不是人工真值，`--complement` 更只是代理的代理；
  兩個數字都只能看改判準前後的**差**，絕對值沒有意義。
- 重放的是 4.5b 的舊逐字稿，判準改了之後**她的生成不會跟著變**——這裡量的是
  「分類器對同一批句子的判法」，不是上線後的端到端效果。
- 舊判準的召回數字取自 4.5c 那一輪（不同 session），新的是這一輪；有一部分差異
  是跑次雜訊，不全是判準的功勞。

### 花費

2,031 次分類器呼叫（315×2 舊 complement ＋ 228×2 召回 ＋ 315×2 新 complement
＋ 315 雜訊重跑），觀測單價 $0.0002027 ⇒ **估價 $0.412**（授權上限 $0.50）。
**這一輪沒有可信的餘額差**：第一次付費呼叫之前沒有先記起始餘額，只能記估價；
下一輪要照 4.5b 的做法先讀一次餘額再開跑。
...
- 逐則結果：`out/2026-09-06-p45f-recall-{standard,assisted}.json`、
  `out/2026-09-06-p45f-complement-{standard,assisted}-{old,new}.json`、
  `out/2026-09-06-p45f-complement-assisted-new-run2.json`（雜訊帶）
