# 練習室「對話主體意識」評測（conversation-agency-v1 Phase 0／1／2／2.5／2.6）
...
**這一輪最重要的結論是「Phase 2.5 的頭條數字有一大半是判準造成的」，不是模型變好或變壞。**
...
逐情境比對同一份 artifact 的 v0 → v1（`adopted_without_asking` ／ `accept_valid_answer`）：
...
新跑 A06／A10／A14／A16（bounded 最重的四個情境，20 位 ×3，600 次生成、零失敗），
兩邊都用 v1 判準比：
...
矛盾時小模型會含糊其辭」。要動的是 planner 對這種輪次判出來的 act 本身
（A21 結構上是 `neutral → acknowledge`），那需要「玩家在抱怨」這個判斷——本輪
照指示不加 regex 偵測器。
...
connection／boundary／coherence 一起丟掉。改成逐字登記 `confused`→`neutral`
的 repair-first 之後就沒有失敗了。**Phase 2.5 待辦第 5 條寫「coherence 判準要
