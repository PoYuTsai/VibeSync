...
目前學習頁的 `AI 實戰練習室` 入口太小，像一個附屬 banner。這不符合產品重心。學習 tab 的第一視覺應該是「聊天練習」，文章練習專區可以往下滑才看到。

新入口方向參考第二張截圖：

- 進入 `學習` tab 後，第一屏主角是 AI 實戰練習室 hero。
- Hero 使用一張陪練女孩照片或多張照片拼接作為背景，背景要大、柔焦、暗化，不要只是一張小橫條。
- 中央放半透明深色玻璃感圓角膠囊，包含：
...
- Hero 可以有 VibeSync 橘色符號或現有 practice icon，但 icon 不能搶過主標。
- 整張 hero 都可點，點擊進入 `/practice-chat`。
- 首屏下方只需要露出一點 `練習專區` 或文章 grid，表示下面還有內容。文章不是第一優先。
- 這個 hero 不是 paywall，也不是翻牌本身；它是進入練習室的主入口。

尺寸建議：

```text
mobile hero height: 58-68vh
desktop/tablet: 可用較寬比例，但仍保持首屏主視覺
```

設計重點：

- 第一眼要讓使用者知道「學習頁重點是聊天練習」。
- 不要像目前的窄橫條入口。
- 背景人物要柔焦，避免使用者誤以為在此頁已經抽到這位女孩。
- Hero 文案要清楚、直接，不加一大段功能說明。
...
- 學習 tab 第一屏主視覺是 AI 實戰練習室 hero，不是小橫條入口。
- 文章 `練習專區` 在學習頁要排在 hero 後方，需要下滑才完整看到。
