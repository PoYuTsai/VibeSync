...
**Goal:** 從 `CoachChatCard` 抽出 scope 參數化的共用 `CoachSurface`，對象頁改掛同一聰明教練 engine（partner scope＝串流/多輪/釐清/成效首次落地夥伴層）；三顆新情境 chip 種入 `lifecyclePhase`＋prefill；client 全路徑送 `requestId`（重送同值）接上 Phase C ledger replay。**exactly-once 範圍註記（Task 8 Codex R1）**：requestId session 只活在 controller instance 記憶體，涵蓋同畫面重試；離開頁面/dispose/重啟後的 lost-response 重送仍會鑄新 id（snapshot 2026-07-18 已知債不變，不得宣稱完整 exactly-once）；durable envelope 持久化記 Phase F 清單。
...
> **Task 7 雙審結果（2026-07-22，spec ✅＋品質 Approved）**：零 Critical。品質審 Important 1 項判定不擋本 task、記入 **Phase F 退場清單**一併處理：
> 1. `coach_follow_up_section.dart` `_didAutoFocusCoachInput` 閂鎖在 partnerId 原地切換時未重置（parent/orchestrator 兩層守衛已做、section 第三層缺；真實 deep-link 走 push 新 route 打不到，防禦一致性補一行即可）。
> 2. `CoachFollowUpInvokedEvent(phase: openCoach)` 語意已從「實際觸發生成」前移為「deep-link 意圖」，接真 analytics 前應改名（如 openCoachIntent）。
> 3. `openCoachInputOnFirstBuild` 參數名已名不符實（主要路徑是中途 false→true transition），Phase F 一併改名。
> 4. **（Task 8 Codex R1 P2）** `CoachRequestIdSession` 純記憶體＋controller autoDispose：跨 dispose/重啟的 lost-response 重送鑄新 requestId、錯過 ledger replay 可能重扣。拍板走「收斂宣稱＋記債」：Goal 措辭已改（見頁首）、snapshot 已知債條目維持；durable envelope 持久化（billable 呼叫前落 Hive、本地結果落定才清）為獨立設計案，Phase F 或另案處理，不在收尾輪 rework。
>
> **清償狀態（2026-07-22 Phase F）**：1–3 已於 Phase F 清償（見 `docs/plans/2026-07-22-coach-unification-phase-f-cleanup.md`），4 維持獨立案。
