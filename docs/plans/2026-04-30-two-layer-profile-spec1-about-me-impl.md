...
>
> **Status:** plan draft, awaiting Codex plan review.
> **Date:** 2026-04-30
...
**Architecture:** Clean Architecture feature folder `lib/features/user_profile/`，加密 Hive box（與 `Partner` / `Conversation` 同一把 AES key）儲存單一 `UserProfile` 紀錄（key = `'me'`），Riverpod `AsyncNotifier` 暴露 `UserProfileState`，UI 走 GoRouter `/profile/about-me`。`UserStyle` (session_context.dart, typeId=7) 不動 — 新建獨立 `InteractionStyle` enum；§13 已明示「不 migrate 舊 SessionContext.userStyle」。
...
class UserProfile {
  ...
  const UserProfile._({
    ...
    required this.topicSeeds,
    required this.customTopics,
    required this.notes,
    required this.updatedAt,
  });

  /// Validates and normalizes inputs. Use this from controllers / repository
  /// rather than calling the private constructor directly.
  ...
  UserProfile copyWith({
    Object? interactionStyle = _sentinel,
    List<PracticeGoal>? practiceGoals,
    List<TopicSeed>? topicSeeds,
    Object? customTopics = _sentinel,
    Object? notes = _sentinel,
    DateTime? updatedAt,
  }) {
    return UserProfile.create(
      interactionStyle: interactionStyle == _sentinel
          ? this.interactionStyle
          : interactionStyle as InteractionStyle?,
      practiceGoals: practiceGoals ?? this.practiceGoals,
      topicSeeds: topicSeeds ?? this.topicSeeds,
      customTopics: customTopics == _sentinel
          ? this.customTopics
...
> **Note:** `UserProfile` 因為帶 named factory + `_sentinel`，需要私有 default constructor 給 hive_generator 用。最簡作法：把 `UserProfile._` 改成位置式 + 加一個 `UserProfile.hive(...)` 給 generator。具體 generator 兼容 pattern 可參考 `lib/features/partner/domain/entities/partner.dart` (typeId=8) — 那邊也是 immutable + factory。若 generator 報錯，回頭把 entity 改成單一公開 const constructor，把驗證移到 repository.save() 入口。
...
**Goal:** 提供 `load()` / `save(profile)` / `clear()` 三個操作，包驗證錯誤 + Hive 失敗成 domain exception。
...
void main() {
  ...
  test('save() then load() returns same profile', () async {
    final p = UserProfile.create(
  ...
  test('save() overwrites previous profile', () async {
    await repo.save(UserProfile.create(
      interactionStyle: InteractionStyle.gentle,
...
/// Single-record local store for the global About Me profile.
///
/// Storage key is the constant `'me'` — there is exactly one profile per
/// install. Box is encrypted by the same AES key used for Conversation /
/// Partner (StorageService).
class UserProfileRepository {
  ...
  Future<UserProfile?> load() async => _box.get(_key);
...
**Goal:** `userProfileRepositoryProvider` (Provider) + `userProfileControllerProvider` (AsyncNotifier with `save / clear`)，UI 透過 `ref.watch` 取狀態。
...
void main() {
  test('initial load with empty repo emits null state', () async {
    ...
    final state = await container.read(userProfileControllerProvider.future);
    expect(state, isNull);
  });

  test('save() persists profile and updates state', () async {
    final repo = _FakeRepo();
    final container = ProviderContainer(overrides: [
      userProfileRepositoryProvider.overrideWithValue(repo),
    ]);
    ...
    final state = container.read(userProfileControllerProvider).value;
    expect(state?.interactionStyle, InteractionStyle.humorous);
  });

  test('clear() removes profile and state becomes null', () async {
    final repo = _FakeRepo()
      .._stored = UserProfile.create(
        interactionStyle: InteractionStyle.steady,
        updatedAt: DateTime.utc(2026, 4, 29),
      );
    final container = ProviderContainer(overrides: [
    ...
    await container.read(userProfileControllerProvider.future);
    await container.read(userProfileControllerProvider.notifier).clear();

    expect(repo.clearCount, 1);
    expect(container.read(userProfileControllerProvider).value, isNull);
  });

  test('save failure surfaces as AsyncError but state preserved', () async {
    ...
    await container.read(userProfileControllerProvider.future);
    expect(
      () => container.read(userProfileControllerProvider.notifier).save(
            UserProfile.create(
              interactionStyle: InteractionStyle.gentle,
              updatedAt: DateTime.utc(2026, 4, 30),
            ),
...
class _FakeRepo implements UserProfileRepository {
  ...
  UserProfile? _stored;
  @override Future<UserProfile?> load() async => _stored;
  @override Future<void> save(UserProfile p) async => _stored = p;
  @override Future<void> clear() async => _stored = null;
}
...
### Step 4: Diff sanity (no scope creep)

```bash
git diff --stat origin/main..HEAD -- lib supabase
```

預期 **沒有** `supabase/` 改動、沒有 `lib/features/analysis/` 改動、沒有 `lib/features/partner/` 改動（除 import 新 enum 的零星調整以外）。**有就停下，重新檢查。**
...
## 12. Open Questions Held For Codex Review

下列題目寫 plan 時保守決定，等 Codex review 時可挑戰：

1. **`UserStyle` 重複問題**：Spec 1 用全新 `InteractionStyle` enum（label 無「型」後綴）vs. 重用既有 `UserStyle`（label 改成無「型」）。本 plan 選前者。Codex 若認為應 alias / typedef，請在 review 提出。
2. **Hive box name**：本 plan 用 `'user_profile'` + key `'me'`。Codex 若認為 multi-account（同 device 多 Supabase user）需 ownerUserId 分隔，請建議 — 但 §11 design 只說 single-record，本 plan 與其一致。
3. **MyReportScreen 重構深度**：本 plan 把 `_buildEmptyState` 拆成 `_emptyStateContents`，Codex 若認為應整段 rewrite into `CustomScrollView` 才能漂亮容納 About Me card + empty / filled 兩態，請另開 follow-up，不在 Spec 1 內擴張。
4. **`profile_chip_section.dart` 抽 helper 是否 over-engineering**：3 個 chip group share 90% 邏輯，本 plan 認為值得抽。若 Codex 認為三段直接展開更清晰，可 inline。
5. **既有 conversation 測試破不破**：手動輸入測試若有 assert `_userStyle` chip 存在，會在 Task 8 紅。本 plan 假設 「破了就改」 而非 「破了就刪 assertion」。
