...
**這份文件解決什麼**：第一批 20 張（PR #30）補的是 `moments_image_catalog.ts` 已宣告的 id——每個 tag 至少有了一張圖。但覆蓋分析（§0）顯示 10 個 tag 只有**一張**圖：下雨天全台北 100 位角色配的是同一扇窗、看書全是同一個書櫃。第二批把最薄的 tag 各加 1-2 張，讓跨角色撞圖不再一眼看穿。
...
1. 1 張的 tag 各補至少 1 張（sky 除外——`moment_sunset_sky` 是刻意的單張晨昏兩可天空，再加一張天空反而稀釋）。
2. 高頻題材（食物、通勤、居家 calm）多補，因為它們每天出現最多次。
...
第一批規格書 §3 的八條**全部適用**（無人、無可讀文字、時間讀不出來、台灣感、手機隨手拍、深色 UI 上好看、240px 可讀、不重現既有作品）。本批的差異只有一條：

- **本批零人物例外。** 第一批的兩個例外（`street_night` 遠處模糊路人、`live_stage` 觀眾剪影）都是那兩張獨有的；本批 20 張全部不得出現任何人形，包含剪影與倒影。

特別提醒三個本批的高風險點：

1. `moment_temple_incense`：廟宇的匾額、燈籠、籤詩櫃**全是字**。字全部失焦、拍局部角落、或乾脆不入鏡。
2. `moment_bubble_tea_cup`：現實中封膜一定有印刷。**生成無印刷的素面封膜**，杯身也不得有logo。
3. `moment_bento_lunch`／`moment_street_food_stall`：包裝紙、紙袋、攤車招牌不得有可讀的字。

---

## 4. 共用前綴與負面詞（與第一批相同，直接複製）
...
### 2. `moment_pour_over_kit`
- **tags**：coffee, calm ｜ 13 個題材（含所有 calm 居家題材與 `cafe_hunt` 的「手沖成果」）｜全時段
- 重點是「**自己在家沖**」，不是店家吧台。

```
A home pour-over coffee setup on a plain kitchen counter: an unbranded ceramic
dripper sitting on a small glass carafe half filled with fresh coffee, a slim
gooseneck kettle beside it, a few stray coffee grounds and a crumpled filter
paper on the counter. Steam rising gently. Soft indoor light, no window in
frame, everything plain and unbranded, slightly messy and clearly home-made.
```

- **驗收**：看得出是家裡檯面不是咖啡店吧台；濾杯、壺、包裝全部無字。

---

### 3. `moment_breakfast_shop_table`
- **tags**：food, cafe ｜ 9 個題材（第一杯、午餐、晚餐、宵夜、週末早午餐、覓食…）｜全時段
- 台式早餐店的桌面。food tag 會讓它偶爾落在晚餐題材——台式早餐店的蛋餅當午晚餐在台灣完全成立，不必迴避。
...
steel table. Tight framing on the tabletop only — no walls, no menu, no signage,
no window. Slightly greasy lived-in table surface, casual and unstyled.
```

- **驗收**：美耐皿盤＋不鏽鋼桌的台式早餐店質感；畫面裡沒有菜單、價目表或牆面。

---

### 4. `moment_bento_lunch`
...
it. Canvas awning shade above so no sky is visible. Any background completely
blurred with no readable signs. Tight, appetite-driven framing.
```

- **驗收**：不拍天空、看不出白天晚上；攤台與紙袋上沒有任何可讀的字。

---

### 6. `moment_bubble_tea_cup`
...
### 12. `moment_headphones_lamp`
- **tags**：music, calm ｜ 9 個題材（回家癱、週末耍廢、追劇、單曲循環、下班…）｜全時段
- 讓 `live_music` 題材的「最近單曲循環的歌」有圖可配（第一批只有現場那張）。
...
### 15. `moment_ridge_city_view`
- **tags**：outdoor, nature ｜ 3 個題材（海邊的一天、上山、花店的花材日）｜morning-early_evening
- 象山／陽明山視角的「爬上來看一眼」，但**不能有可辨識地標**（不得出現 101 形狀的塔樓）。

```
A hazy view from a mountain lookout above Taipei: layered green subtropical
ridges in the foreground, a dense field of ordinary city blocks dissolving into
haze far below. Overcast diffuse light, muted grey-green palette, no sunset
colors. No landmark towers or recognizable skyline silhouettes — just generic
rooftops in haze. A blurred edge of a wooden railing at the frame corner.
```

- **驗收**：認不出是哪座山哪個城市（無指標性建築）；霧面平光不是日落大景。

---

### 16. `moment_park_bench_shade`
- **tags**：walk, outdoor ｜ 6 個題材（下班散步、週末出門、隨手拍、海邊、上山、夜晚散步）｜全時段
- 「走一段路坐一下」的那張椅子。樹蔭平光＝時段不可讀。

```
An empty park bench under the deep shade of an old banyan tree in a Taipei
neighborhood park: worn green paint flaking on the bench slats, a few fallen
leaves on the seat, a paved path curving away behind. Flat even shade light so
the hour cannot be read, background foliage softly blurred. No people, no
signage, no readable park notices.
```

- **驗收**：長椅有掉漆的使用感；樹蔭平光看不出時段；無告示牌文字。

---

### 17. `moment_riverside_track`
...
### 18. `moment_cat_loaf`
- **tags**：pet, calm ｜ 9 個題材（回家癱、週末耍廢、追劇、家裡那隻、下班…）｜全時段
- 與第一批 `moment_cat_nap`（虎斑白、沙發熟睡）必須是**不同隻、不同姿勢**；室內無窗以免洩漏時間。

```
An orange tabby cat sitting in a loaf position on a woven rug on a wooden
floor, eyes half closed, front paws tucked under. A blurred corner of a fabric
sofa behind. Soft warm indoor lamp light, no window in frame. The cat is a
common house cat, slightly chubby, photographed from a low casual angle as if
sneaking a photo without disturbing it.
```

- **驗收**：橘貓、母雞蹲——與 `moment_cat_nap` 明顯不同隻不同姿勢；室內無窗。

---

### 19. `moment_balcony_plants`
- **tags**：nature, calm ｜ 9 個題材（回家癱、週末耍廢、上山（花材）、花店、下班…）｜全時段
- 台式公寓陽台：磁磚地＋金屬欄杆，不是歐式花園。

```
A cluster of potted plants on a Taiwanese apartment balcony: mismatched plastic
and ceramic pots on old beige floor tiles, monstera and small ferns, a plain
metal watering can, part of a painted metal railing at the frame edge. Overcast
diffuse daylight, everything slightly weathered and humid. Tight framing on the
plants and tiles, no cityscape beyond, no laundry, no people.
```

- **驗收**：磁磚＋鐵欄杆的台式陽台感；花盆混搭不成套（不是園藝店陳列）；不拍到外面天空街景。

---

### 20. `moment_sketchbook_desk`
...
for id in moment_iced_latte moment_pour_over_kit moment_breakfast_shop_table \
  moment_bento_lunch moment_street_food_stall moment_bubble_tea_cup \
  moment_baking_tray moment_rainy_alley moment_bus_window_rain \
  moment_scooter_helmet moment_open_book_blanket moment_headphones_lamp \
  moment_harbor_boats moment_temple_incense moment_ridge_city_view \
  moment_park_bench_shade moment_riverside_track moment_cat_loaf \
  moment_balcony_plants moment_sketchbook_desk; do
...
人眼檢查（與第一批同一套）：每張縮到 240px 還說得出是什麼；40 張擺在一起是同一家人的色調；放深紫底 `#1F1330` 上不刺眼、無大面積純白；逐張核對 §5 的驗收行。
...
尺寸與總 bytes、人工檢查描述（可辨識文字／第三方作品／品牌標識未發現）、
...
1. **`moments_image_catalog.ts`**：`MOMENT_IMAGES` 追加 20 筆（tags 照 §5；全部使用既有
   ...
   { id: "moment_pour_over_kit", tags: ["coffee", "calm"] },
   ...
   { id: "moment_ridge_city_view", tags: ["outdoor", "nature"] },
   { id: "moment_park_bench_shade", tags: ["walk", "outdoor"] },
   { id: "moment_riverside_track", tags: ["fitness", "outdoor"] },
   { id: "moment_cat_loaf", tags: ["pet", "calm"] },
   { id: "moment_balcony_plants", tags: ["nature", "calm"] },
