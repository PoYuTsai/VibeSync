...
> **題數規格已過期（2026-08-02）**：本檔的「4 關 31 題／免費關 10 題／付費關 6–8 題」已由 ADR #36 取代，
> 現況是四關等量 15 題、共 60 題。內容真相源改為 `2026-08-02-chat-quiz-1-1-content-redesign.md`
> 與 `2026-08-02-chat-quiz-1-2-2-1-2-2-content-redesign.md`。本檔其餘工程結構仍然有效。
...
**更正 4 — 權限規則與內容表互相衝突（實作時發現，已按下述方式解）。**

原規則寫「一關的 `access` ＝ 所有題目來源書當中最高的那一層」，但同一份計畫的內容表寫 1-1 是 **free** 而取材包含**第 2 冊（premium）**。兩者不可能同時成立。另外 1-2 原訂取材第 7 冊 3.2／3.5，但第 7 冊是 Essential，那會讓 1-2 依決定 5 必須升成 essential，Starter 就整群進不去，與內容表的 premium 相矛盾。

處理（都往「不動 Bruce 的產品決定」的方向走）：

1. **權限規則收斂成真正的紅線**：取材第 5–7 冊 ⇒ 必須宣告 `essential`（決定 5）；其餘關卡的宣告值不得低於推導值；**唯一例外是免費櫥窗關 1-1**，它刻意用付費書取材，但取材不得超過 premium。理由：免費關的「讀原理」按鈕點下去打的是電子書自己的付費閘門，看到的是 paywall——那正是櫥窗該有的轉換路徑，不是內容外洩。
2. **1-2 改為取材第 3 冊（3.1 診斷樹／3.3 疑難情境）＋ 第 2 冊拆解庫 G／H／I（救援三案）**，維持 premium。這兩處有現成且更貼題的素材：案例 I 直接寫了「最近比較忙 ＝ 委婉拒絕 → 先不吵你 → 再沒有 → 結束，這是紅燈」，§7.5 第 3 條與 §7.6 第 19、21 條（拒絕訊號三類、停損）都落得下去。第 7 冊的題目留到第 2 期，屆時可另開 essential 關。
...
| 1-2 消極期先止損 | 7 | premium | 讀燈 ×4、選回覆 ×3 | 第 3 冊 3.1／3.3 ＋ 第 2 冊拆解庫 G／H／I（含**拒絕訊號三類**與**停損**；見 §0 更正 4，原訂的第 7 冊改到第 2 期）|
