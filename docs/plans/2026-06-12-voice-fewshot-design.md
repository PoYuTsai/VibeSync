...
2. **新增 2 個完整 voice few-shot**（輸入對話 → 完整輸出），**每個範例標明關係階段**：
   - 範例 1 = 糖糖老師場景（ChatGPT golden 轉寫成 segments 格式，**Eric 逐句定稿**）：**已升溫／熟絡局**——callback + 推拉 + 幽默。
   - 範例 2 = **交友軟體陌生早期局＋對方偏冷**（真實用戶大宗場景）：示範陌生局的高手聲音——精準觀察讓人眼睛一亮、輕、低壓、不裝熟、收斂不耍熟絡式幽默。一個範例同時防過擬合＋覆蓋主用戶場景。

   **核心原則（Eric 2026-06-12 補拍板）**：「高手感」≠「幽默感」，它隨關係階段縮放。golden case 前提是 Bruce 與已熟絡伴侶的對話；真實用戶大宗是交友軟體陌生局，照搬裝熟幽默會 creepy。現有 12 場景判斷區負責「選檔位」（**這塊是資產不是稅，不砍**）；few-shot 負責「每個檔位裡的高手聲音」。
...
**3-case 盲測**：golden case（糖糖老師，熟絡局）＋**陌生早期局**＋**陌生冷淡／試探局**（後兩者取自真實 dogfood，向主用戶場景加權——陌生局佔 2/3）。每 case 三版輸出：舊 prompt（prod baseline，先跑留檔）／新 prompt／ChatGPT。去識別、隨機排序，Eric 盲選排序。陌生局的「不輸」判準：眼睛一亮但不裝熟、不 creepy。
