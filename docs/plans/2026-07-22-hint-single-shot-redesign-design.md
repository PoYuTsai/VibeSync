...
| 明文禁止 PUA／情緒勒索 | prompt 規則＋機械 L4 守門 | 零／毫秒 | **保留**（CC 裁決：零延遲收益、App Review 期拆防線＝上架與品牌風險；reviewer 刪除後 prompt 規則是唯一防線） |
| 不准亂編（事實接地） | prompt 規則＋hint_fact_ledger 機械檢查 | 毫秒 | **保留**（同上） |

Eric 曾提議「全部放寬」；CC 反對理由如上——限制不是慢的原因，快慢帳上是免費的。若 Eric 堅持再改此節。
