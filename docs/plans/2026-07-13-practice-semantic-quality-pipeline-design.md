...
目標 budget：常態兩次模型呼叫；最差三次。Hint server wall budget 60–70 秒、client 等待 75–90 秒、in-flight stale owner 必須大於 client timeout。精確值以 provider probe P95 與 Supabase wall limit決定，不靠猜。
...
9. **Bounded cost**：每 surface 每個 request 最多三次 provider call；不得無限自我修復。
...
| 三次 provider budget 用完仍不合格 | retryable 503；不 record、不扣、不計、不回罐頭 |
...
| client 90 秒先 timeout、server 後成功 | 視為 regression；Debrief stale owner 105 秒，先保住同 requestId replay |
...
- semantic adjudicator parser、provider failover、repair、三次 budget exhaustion。
...
1. 先以測試鎖定 semantic schema、hard guard、三次 call budget、計費與 Debrief lineage。
...
4. client Hint／Debrief timeout 90 秒，Debrief stale owner 105 秒；先部署 owner migration，再部署 Edge。
