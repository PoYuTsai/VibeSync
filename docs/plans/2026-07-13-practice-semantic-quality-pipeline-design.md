...
現行 bounded budget（2026-07-19 修正）：Hint 最多五次 provider call；Debrief 常態為一次 generation＋full reviewer＋fact verifier，初輪 reviewer envelope 失敗時會預留 Claude regeneration＋同樣兩個 reviewer，最多六次。Debrief 所有 generation／reviewer call 共用從 request entry 起算的 85 秒 deadline；client 90 秒、in-flight stale owner 105 秒。Hint client 維持 115 秒。
