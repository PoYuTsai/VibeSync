...
目標 budget：常態兩次模型呼叫；generation failover 後仍保留兩個 reviewer slot，最差四次。Reviewer timeout 18 秒，client 等待 90 秒、in-flight stale owner 必須大於 client timeout。
...
9. **Bounded cost**：每 surface 每個 request 常態兩次、最多四次 provider call；不得無限自我修復。
...
| 四次 provider budget 用完仍不合格 | retryable 503；不 record、不扣、不計、不回罐頭 |
...
- semantic adjudicator parser、provider failover、repair、四次 total budget exhaustion。
