...
- Archive repository／intent／分流／獨立頁／舊 Partner tests：通過。
- OCR service + dialog：28 tests 通過，targeted analyze 無 issue。
- Collection screen：33 tests 通過，targeted analyze 無 issue。
- Draw handler：`deno test --allow-env .../draw_handler_test.ts` 18/18；
  `deno check`、`deno fmt --check` 通過。
- 出貨狀態：等待整包回歸、Codex 雙審、Edge deploy 與新 TestFlight build；在雙審
  APPROVED 前不得宣稱 dogfood-safe。
