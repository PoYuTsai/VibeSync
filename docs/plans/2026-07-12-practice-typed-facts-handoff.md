...
- 驗證：`deno fmt --check` 6 檔、`git diff --check -- . ':!pubspec.lock' ':!.gstack'`、`deno test --allow-env --allow-read --no-check supabase/functions/practice-chat` = 857/857 passed。下一步：commit/push 後部署 `practice-chat` Edge，再跑 `.gstack/investigations/practice_generated_only_production_smoke.ts` 作為 dogfood safe 依據。
