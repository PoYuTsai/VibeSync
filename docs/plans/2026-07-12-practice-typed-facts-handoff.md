...
- 測試已補：`debrief_card_test.ts` 驗證 production repair 只在 opt-in 時生效，且 Game breakdown 同樣可通過 grounding/role/route guard；`index_test.ts` 驗證 DeepSeek 卡可被 deterministic repair 後直接成功，且 missing hidden assessment 的雙 provider 仍 503、不 record card。驗證：`debrief_card_test.ts` 69/69、`index_test.ts` 197/197、practice-chat 全套 855/855，fmt/diff check 全綠。
