...
- 下一步：只 stage practice-chat 第三修補＋本 handoff＋bug-log；不要 stage `pubspec.lock` 或 `.gstack/`。commit/push 後 deploy `practice-chat` Edge，再跑 `tools/practice-chat/practice_generated_only_production_smoke.ts`。若 smoke 過，回報 server-only Edge deploy；若仍 503，查 ai_logs 的 sanitized `failureCodes` 再修。

## 2026-07-13 Codex takeover fourth follow-up

- 第三修 deploy 後 smoke 的 Beginner Hint 已 generated，但 Beginner Debrief 仍連續 503，ai_logs 顯示兩個 provider 都卡 `debrief_hint_assessment_revision_required`。這代表 hidden contract 已存在，但可見拆盤仍把 preserved exact Hint 批評成「太保守／禮貌收尾／沒給球」。
- 修法：只在 generated Debrief opt-in `repairPreservedHintCritique`；條件必須同時滿足：已套用 Hint、hidden `hintAssessment.verdict="preserved"`、`revisedEvidenceQuote=null`、可見欄位確實批評/反轉 Hint。server 取 Hint 後第一則 AI 回覆當 grounding，改寫 summary/strengths/watchouts/dateChanceReason/nextInviteMove 與 Game breakdown 為「她有接住 → 下一步補生活畫面」；再跑原有 `assertHintAssessment` 與 generated-quality guard。缺 hidden assessment 或不能 grounding 仍 fail-closed，不落 fallback 卡。
- 本地 review 抓到 Game breakdown repair 的完整性缺口：拆盤欄位若沒帶 post-Hint AI quote 或不符合 phase/variable/failure/invite role guard，Game 仍會 503。已改成每個 Game repair 欄位都帶她接住的逐字稿錨點，並避開會被 route parser 誤判的「邀約」否定句。
- 第一次 deploy 後 smoke 仍卡 `debrief_hint_assessment_revision_required`；ai_logs 顯示已不是 missing hidden 或 token，而是 preserved hidden + 可見策略反轉。補強：`cardContradictsHintStrategy` 也會觸發 repair，且同步改寫 `suggestedLine` / `gameBreakdown.nextFirstLine`，避免可貼句直接升邀約或覆蓋 preserved build Hint。
- 測試已補：`debrief_card_test.ts` 驗證 production repair 只在 opt-in 時生效，且 Game breakdown、suggestedLine、nextFirstLine 同樣可通過 grounding/role/route guard；`index_test.ts` 驗證 DeepSeek 卡可被 deterministic repair 後直接成功，且 missing hidden assessment 的雙 provider 仍 503、不 record card。驗證：`debrief_card_test.ts` 69/69、`index_test.ts` 197/197、practice-chat 全套 855/855，fmt/diff check 全綠。

## 2026-07-13 Codex takeover fifth follow-up

- 第四修後 production smoke 進到下一個真問題：Beginner Hint/Debrief 已可 generated，但 Game Hint 在「她問哪家咖啡店」場景連續 503。ai_logs 顯示 DeepSeek 常先 `deepseek_timeout`，Claude failover 後被 typed-facts 守門殺在 `hint_quality_invalid_unsupported_detail:world:venue:located_at` 或 `hint_quality_invalid_pure_questions`。
- 根因：Game Hint prompt 沒吃到 Beginner/非 Game 那條「未提供店名/地點不能捏造」紅線；同時 asksPlace 守門對「那家/那間/店名沒記」這種未命名店家承接太保守，會把正確的不捏造回答誤殺。這不是放回 fallback，而是讓模型在被問店名時產生「我沒記店名，只記得感覺」這種 grounded 回覆。
- 修法：Hint provider timeout 12s→18s（Eric 明確接受犧牲等待時間降低 fallback/503）；Game/Beginner 共用未提供店名/地點/共同經歷別捏造規則；fact ledger 只放行逐字稿中已有「一家店/那家店/那間店」的未命名 venue carryover，具體店名、地址、地標仍 fail-closed。
- 驗證：`deno fmt --check` 6 檔、`git diff --check -- . ':!pubspec.lock' ':!.gstack'`、`deno test --allow-env --allow-read --no-check supabase/functions/practice-chat` = 857/857 passed。下一步：commit/push 後部署 `practice-chat` Edge，再跑 `tools/practice-chat/practice_generated_only_production_smoke.ts` 作為 dogfood safe 依據。
