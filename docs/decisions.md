...
>2000 字    → 跳「額外計費通知」，用戶確認後才扣 >10（見 OPEN-1）
```

**預覽 UX（覆寫 r2「分析前即時精確數字」）**:
- 分析前**不算精確值**，顯示固定區間文案：「會依對話複雜度使用 1–10 枚」。→ 免 client pre-flight 字數計算、免兩端預覽 mirror 爭議。
...
**🟠 OPEN — 待 Eric 一句話確認（實作前必補）**:
1. **>2000 字確認後怎麼扣**：(甲) 全額 `ceil(字數/40)`（2000 字 = 50 則…） vs (乙) 第二段硬 cap（如最多 20 則）+ 引導分批。**CC 建議 (乙)** —— 單次 50~75 則易燒光月額度。
2. **40 字制 = 既有用戶 quota 燒快約 5 倍**（200→40）。須對照 `docs/pricing-final.md` 月額度確認：同步調額度，或確認現有額度撐得住。**未確認前不得部署。**
3. **單位用詞**：夥伴寫「枚 Token」。內部/計費維持「則」。**CC 建議**用戶端文案沿用既有「則」、**不引入「Token」**（易與 AI token 成本混淆）。待 Eric 拍板。
