...
## ADR #30 — [2026-08-18] 產品定位：搶「AI 戀愛教練」心智第一，差異化「帶你從曖昧走到約出來」
**Status**: Active

**Decision**: 品類詞全產品統一「AI 戀愛教練」（不用「約會教練」）；核心差異化為「別人解決這一句怎麼回，只有我們負責這一段怎麼走完」；「記憶」只作信任狀不作口號，對外寫成「它認得你聊的每一個她」；打法為側翼戰（打回覆鍵盤類的不可逆弱勢），不拚價格、不蹭 AI 伴侶聲量。完整論述、文案系統與 App Store 待辦見 `docs/positioning.md`（方法論參考顧軍輝定位講堂筆記，路徑記於該檔）。
...
## ADR #36 — [2026-08-02] 聊天測驗內容重做：四關等量 15 題（推翻「免費關 10／付費關 6–8」）

**狀態**: 🟢 Active

**背景**: 夥伴依男女心理學與社交力學重做第 1 期題庫，出兩份設計文件（`docs/plans/2026-08-02-chat-quiz-1-1-content-redesign.md`、`docs/plans/2026-08-02-chat-quiz-1-2-2-1-2-2-content-redesign.md`）。診斷：原 31 題只有極少數合乎真實社交力學，其餘太「乾淨」、太教科書；其中 1-1 的 Q09（辭職唸書開場）要求陌生人剖析情緒，正好違反它自己深連的教材（第 1 冊第 4 章）對「開口成本要低」的定義，是客觀瑕疵而非主觀偏好。Eric 拍板照文件重做。

**決定**:

1. **四關等量 15 題、共 60 題**，推翻原本「免費櫥窗關 10 題密度最高、付費關 6–8 題」的前提。內容不變式測試改為每關固定 15。
2. **內容全面重寫**：每題鎖一個反直覺／常被誤讀的訊號（三燈長相、廢物測試、情緒 vs 字面、速度 vs 內容、合理化引擎、匱乏心態、張力型 vs 焦慮型波動、三死因、側面價值、回應性）；情境全面台灣化（Tinder／IG 限動／LINE 已讀），移除 Q09。
3. **正解位置刻意分散**，不再一律排第一個。連帶的鐵則：任何測試都不得再假設「第一個選項＝正解」，要從 catalog 查真實正解位置（`chat_quiz_proof_test.dart` 已改）。
4. **權限與取材不變**：1-1 free、其餘 premium；取材維持第 1–4 冊。設計文件提出的「為第 7 冊深料把某關升 essential」本次不做——那會動付費牆，需要另外拍板。
5. **全部題目 `revision` 升到 2**，舊作答紀錄自動失效需重答（沿用電子書 quiz 語意）。
6. 四關分工矩陣（1-1 讀燈／1-2 停損／2-1 死因／2-2 情緒開口）依設計文件第 7 節，避免重疊。

**驗證**: 內容不變式自檢（60 題、每關 15、每題 ≥3 選項恰好 1 正解、每選項有回饋、source 全部指向真章、免費關不取材第 5–7 冊、1-2 有停損正解、正解無操縱語彙、首末題非最難題型）全過；learning 單元＋widget 349 綠、chat quiz visual proof 6 綠、Dart analyze 0 issue、全套測試綠。

---

## ADR #37 — [2026-08-07] 刪帳號本地清理語意：維持裝置級全滅（clearAll）

**狀態**: 🟢 Active

**背景**: 2026-08-07 全專案稽核 #7 指出兩個問題：(1) 刪帳號的本地清理走 `StorageService.clearAll()`＝裝置級全滅，同一支手機曾登入過的其他帳號本地資料（對話、Partner、練習紀錄）一併陪葬；(2) 本地清理中若 App 被強殺，沒有重放標記，清理永遠不會完成，前用戶資料躺在裝置上等下一個帳號。

**決定**（Eric 拍板）:

1. **維持裝置級全滅**。理由：符合 Apple 帳號刪除審查精神、隱私上最乾淨、零新工程；產品形態一人一機一帳號，同機多帳號誤傷是幾乎不存在的情境。「改逐帳號清理」需為所有 box 補 owner scoping，工程量大、收益趨近零，已否決。
...
## ADR #39 — [2026-08-08] Game Mode onboarding：client 靜態教學文案可用原始術語

**狀態**: 🟢 Active

**背景**: Game Mode 機制層（五相 FSM／四變數／失敗診斷／Hint／拆盤）已完整，但沒有事前教學——使用者零框架知識進場，只能靠扣額度的 Hint 與局末拆盤被動學。本次補上一次性教學 Bottom Sheet＋對話內教練開場訊息（純 client 靜態文案＋UI）。

**決定**（Eric 拍板）:

1. 教學文案**教原始術語**（框架/推拉/篩選/DHV/可得性），並附 glossary 對照（框架→App 提示顯示「節奏與主見」、推拉→「輕鬆張力」、DHV→「生活樣本」、篩選→「互相合適度」），讓學到的原詞能接上 Hint 實際用語。
2. 此決策**只涉 client 靜態文案**：server 端 1.2 安全詞表與 `repairChineseJargon` 可見詞轉譯規則不變、範圍不擴大（Codex 審過的 hint/debrief 轉譯照舊）。
...
## ADR #42 — [2026-08-21] Android 首次上架測試採「付費補門檻＋台灣真實使用者驗品質」

**狀態**: 🟡 In progress（roundtable／規格／計畫已完成；Play 開發者帳戶設定與分段實作依獨立授權推進）

**背景**: 新建 Google Play 個人開發者帳號目前須先完成至少 12 名測試者連續 14 天的封閉測試，並由 Google 另行審查測試互動、回饋與正式版完備性。Roundtable 前 Eric 尚未確認可用的 Android 實機；VibeSync 又是繁中 AI dating coach，單靠海外泛用測試者無法驗證實際內容品質。2026-08-22 已確認一台 Android 15 實機可供 Wi-Fi 測試與裝置驗證。

**決定**（Eric 2026-08-21 記錄）:

1. Eric 先自行尋找幾位使用 Android 的台灣朋友，作為繁中內容、真實使用流程與在地體驗的主要測試者。
2. 若最後使用需履行封閉測試門檻的新個人帳號，將 Testers Community 付費方案列為補足人數與持續互動的備選；目前 Pro 標價 £16.99，依當日匯率及海外刷卡費暫估約 NT$750–780，實際購買前重新確認價格、條款與退款政策。
3. 付費測試服務只用於協調封閉測試人力，不取代 VibeSync 自有 QA。正式採用前須準備專用測試帳號、虛構聊天資料、AI 成本／額度護欄，且不得提供 Play Console 密碼、Google 主帳號、原始碼、API key、Supabase 管理權限或簽署金鑰。
4. Google 正式版權限申請須依真實紀錄作答，明確揭露付費測試來源；不得直接照抄第三方提供、可能淡化招募方式的制式答案。
...
## ADR #44 — [2026-08-26] 開場救星交棒改為直接進「新增對象」，不再自動帶入開場白

**狀態**: 🟢 Active

**取代**: 「開場救星 →『接續開場』頁（`/new?source=opener`）自動把上一則開場白塞進『我說』」這條路徑。該頁與其帶入邏輯已整組移除，`/new` 只剩「手動輸入」一種樣貌。

**背景**: 舊路徑上「她回覆了，開始分析對話」會先進一個中繼頁：開場白已預先填進「我說」，使用者要在同一頁再打一次對話對象名稱，建立出來的是一段沒有對象卡的孤兒對話（預設名「開場草稿」）。對象記憶因此接不上——真正要承接後續分析的是對象卡，不是這段孤兒對話。
...
**狀態**: 🟢 Active — 2026-09-03 隨 PR #61 合併生效
...
**附錄（2026-09-24）：照片／自介 Opener 只做開場教練，移除首句邀約代寫**: 來源為 Eric 2026-09-24 的定位澄清（站在教練視角，Opener 的初衷只是幫忙開場；用戶打「請幫我邀約」不代表照單代寫），經 ChatGPT 收斂決策（`ChatGPT_教練型Opener_收斂決策.md`）整理。本附錄明確修訂原規格在陌生開場入口「保留明確邀約意圖／原句動作」的相關文字，不是宣稱原規格本來如此；最小改動提案的 D2（用戶明確要求即可首句邀約）移除，相衝突的舊文字以本附錄為準。

1. **邀約目標只決定從哪件事開聊**：想約她去 X、幫我約、第一句就約、貼上寫給她的邀約原句，五張卡都不提出見面、共同活動或約時間，改從 X 或最接近 X 的有來源話題開場；不因沒寫邀約就換掉 X，也不把目標錯標 irrelevant。不以「直接／幫我／這則」當開關，不新增 UI、模式或模型呼叫。
2. **明確要求時如實說明**：用戶明確要求邀約或貼了邀約原句，最終可見推薦的理由要說明這裡先從 X 開話題；推薦由系統依方案從可見卡選，模型不知道哪張會被推薦，所以五張 `cardReasons` 都順帶帶上（用戶只看得到推薦那張的理由）。說明不寫進可複製的開場句，也不假稱已照寫邀約。
3. **話題優先、背景不強塞**：想聊／想問的 X 仍須接住；家人／朋友的事與用戶過去經歷預設只作選題與語氣依據，不為證明採用寫進句子、不硬抓共同點、不規定自述位置。
4. **提示欄沒有邀約時機義務**：`cardReasons`／`pioneerPlan`／`openingStrategy` 受同一份來源與限制約束；對方明說不見面時所有可見欄位都不建議再約；單寫「不約」保留不確定，不當成可以約。（**2026-09-25 取代**：Eric「不約、不見面很多女生都會這樣寫，還不是約得出來，看階段；Opener 只負責開場，文案要對齊」——「明說不見面不再約／單寫不約保留不確定」兩句自 prompt 移除，提示欄不判斷能不能約；結構刀路徑的邀約說明一律「這句先從「X」開聊／這句先開話題，邀約等她回應熱絡再提。」，見開場救星結構刀需求修訂 B。）
5. **不變**：捏造、否定反轉、排除話題、家人守門、確定度升級、omit 回流與 use/omit 規則等硬守門；Free 三卡／付費五卡、SQL／權益／計費；格式修復＋內容修正共用一次名額、修不好 502 不扣不計次。`OPENER_FLOW_PROMPT_VERSION` 改為 `opener-two-stage-topic-first-v1`（進生成輸入 hash）。
6. **歷史不重寫**：舊測試結果、回放標籤的歷史 miss／exempt 與審查紀錄保留，只加附日期的取代說明；舊邀約斷言依新定義改寫時，原本的 omit 不翻回 use、唯一修正名額、失敗不扣費等保護改以話題型 fixture 保留。原第四輪審查（R4）`BLOCKED`／pilot `NEEDS_FIX` 維持，不開新輪、不重置；`6997656e` 仍是原基準，修改後的候選另行識別。
7. **界線**：本附錄是產品定義，不是程式審查通過，也不是 V2 付費、production 讀取、push／merge／deploy 或發布授權。
