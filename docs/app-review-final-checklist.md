...
- [ ] 單張聊天截圖識別後匯入對話可成功
- [ ] 截圖後直接分析可成功
- [ ] LINE 引用回覆：外層 bubble speaker 判斷正確，引用卡只當 quoted context（2026-09-03 黑箱 65 張：整體持平 7 月基線〔側邊 98.9%、召回 93.5%、引用卡預覽 59%→75%〕；但 8 張引用卡圖裡 2 張暗色 LINE 圖 Sonnet 5 完全不標引用卡→引用文字變鬼訊息、講話者判反，重跑 4 次皆同；同兩張強制 Sonnet 4.6 會正確標出。待 Eric 決定：修 OCR prompt／換 OCR 模型／先靠匯入前手動滑動改側邊）
- [ ] 長截圖可成功（2026-09-03 黑箱：唯一長圖單元 S__5480452 同上受引用卡問題影響；其餘情境 standard 46 張側邊 100%）
- [ ] 多張截圖 overlap 情境可成功
- [ ] 名字小字、錯字、模糊邊界案例已抽測
