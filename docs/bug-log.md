...
**追加（2026-07-13 Codex takeover fourth fix）**: 第三修 deploy 後 production smoke 仍卡 `debrief_hint_assessment_revision_required`；prompt-only 約束不足，模型會在 hidden `hintAssessment.verdict="preserved"` 的同一張卡裡，於可見 summary/watchouts/Game 拆盤繼續批評已套用且被對方接住的 Hint。修法仍維持 fail-closed：只有 generated Debrief 路徑、hidden verdict 明確 preserved、`revisedEvidenceQuote=null`、且可由逐字稿找到 Hint 後第一則 AI 回覆時，才 deterministic repair 可見拆盤，把內容改成「她有接住 → 下一步補生活畫面／追問」；repair 後仍重跑 `assertHintAssessment`、route guard、generated-quality guard。缺 hidden assessment、策略反轉、或無法 grounding 仍 503 retryable，不落 fallback 卡。本地 review 另補 Game breakdown repair：每個拆盤欄位都要帶 post-Hint AI quote 錨點，且文案必須通過 Game phase/variable/failure/invite role guard。第一次 deploy 後 smoke 仍顯示 `debrief_hint_assessment_revision_required`，原因是 production 模型還可能把反轉藏在 `suggestedLine`/`gameBreakdown.nextFirstLine` 或其他 strategy-bearing 欄位；補強後 `cardContradictsHintStrategy` 也會觸發 repair，並同步改寫可貼句與 Game 下一句，避免 direct invite/咖啡邀約覆蓋 preserved build Hint。驗證：`debrief_card_test.ts` 69/69、`index_test.ts` 197/197、practice-chat 全套 **855/855**（`--no-check`），`deno fmt --check` 4 檔與 `git diff --check -- . ':!pubspec.lock' ':!.gstack'` 全綠。

**追加（2026-07-13 Codex takeover fifth fix）**: 第四修 deploy smoke 後 Beginner Hint/Debrief 已 generated，但 Game Hint 在「她問哪家咖啡店」場景連續 503；ai_logs 顯示 DeepSeek 多為 `deepseek_timeout`，Claude failover 後被 `hint_quality_invalid_unsupported_detail:world:venue:located_at` 或 `hint_quality_invalid_pure_questions` 擋下。根因是 Game Hint prompt 少了「未提供店名/地點不能捏造」紅線，模型被問哪家時容易編店名；另一方面 asksPlace typed-facts 對「那家/那間/店名沒記」這種未命名店家承接過度保守，會誤殺正確的不捏造回答。修法：Hint provider timeout 12s→18s（用等待時間換較低 503/failover 率）；Game/Beginner 共用未提供店名、地點、共同經歷別捏造規則；fact ledger 只放行逐字稿已有「一家店/那家店/那間店」時的未命名 venue carryover，具體店名、地址、地標仍 fail-closed。驗證：`deno fmt --check` 6 檔、`git diff --check -- . ':!pubspec.lock' ':!.gstack'`、practice-chat 全套 **857/857**（`--no-check`）全綠；仍需 Edge deploy 後跑 production generated-only smoke 作最終 dogfood safe 依據。

**追加（2026-07-13 final semantic redesign）**: deploy smoke 再揭露兩個同根問題：hidden reviewer strategy 能把「精神快關機」等推測帶進 Debrief；Game Hint 的 full reviewer 又必須產完整 strategies JSON，造成 `semantic_adjudication_invalid_strategy`、repair 無法驗證與 timeout，正式點擊連三次 503。最終解不是再補 regex，而是刪除 semantic result 的整個 strategies 介面：模型只修 `warmUp/steady/coaching`，server 依逐字稿、角色資料與 Game ledger 建 authoritative decision；legacy／捏造／stale reviewer strategies 都無法進 response 或 Debrief。Hint reviewer cap 4,000→1,800、generation 18s→24s、semantic 24s，client 90s→115s；被直接問時先回答／表態，兩案不得只問。最終 Edge Deno **914/914**、Flutter controller **174/174**，`practice-chat` 已部署。Production smoke：Beginner＋Game 正式 Hint 皆第一次成功、replay stable、`fallbackUsed=false`；Game 在 DeepSeek timeout 後由 Claude 62.7s 完成，整體 PASS。Game Debrief 曾一次 fact-verifier 503，same requestId 重試成功，屬 fail-closed 保守拒絕而非罐頭成功。審查：`docs/reviews/2026-07-13-practice-semantic-quality-pipeline-codex-review.md`。

**追加（2026-07-14 Claude-primary 重構）**: live `ai_logs` 顯示 v113 上線後，Game Hint 正式結果 8 成功／20 失敗、Beginner 約 21 成功／21 失敗；命中 dogfood 帳號的兩次 SR Game Hint 都死在 `semantic_adjudication_repair_unverified:semantic_fact_verification_rejected`，耗時 19.1／32.9 秒。根因是同步多模型階段本身：生成成功後仍可能被 repair/verifier 拒絕，增加等待又放大 503。正式預設路徑因此改為 Claude Sonnet 單一 writer：第一次通過既有 schema／grounding／typed-facts／角色／Game FSM／Hint lineage 守門就回傳；只有明確拒絕或暫時錯誤才再呼叫一次 Sonnet，最多 2 次 × 24 秒，不再同步 semantic 二審。Debrief direct path 同時關閉 deterministic 可見文案修補，避免 server 把怪答案改成另一句固定話術；不合法就要求 Sonnet 重寫。一般 chat 仍走 DeepSeek；`PRACTICE_CLAUDE_PRIMARY=false` 只作緊急回滾。兩次失敗仍 503 且不落檔／扣費／計次。local `practice-chat` **920/920**，Codex pre-deploy review 0 P0/P1/P2；production smoke 待部署後補證據。
...
**追加（2026-07-14 語意歸因重構）**：build 323 真機再次出現 Game Hint 兩次「等太久」與 Debrief 失敗；live `ai_logs` 顯示 Claude 已在 12–23 秒內回覆，但三個候選全被 lexical typed-facts guard 以一般人名、家鄉或目前位置判成 `unsupported_detail`，不是 provider 全線 timeout。第一版先把 regex 降成語意審查觸發器，但 deploy smoke 又抓到相反漏洞：對方只問「昨晚看什麼這麼入迷」，Hint 在 regex 零告警下直接替使用者編出《黑白大廚》與《淚之女王》，Debrief 再把錯誤 Hint 當事實肯定。最終修法因此是 always-on：每個 direct Hint／Debrief 候選都固定經一次 `temperature=0` Claude 事實歸因審查，regex 不再決定要不要審、也不再裁決一般人名／地點／經歷；電話／Email／社群帳號仍 deterministic fail-closed。正常路徑固定 writer＋review 兩 calls，總上限三 calls 且任何未審 writer 都不能成功。若 writer 已回候選但撞格式、安全或 Hint 連動契約，下一 call 直接在同一次 review 修契約並審事實，不再浪費一格盲寫；只有 provider 完全沒回才重叫 writer。後續 smoke 又證實 direct Debrief 的「只問／只回／片段重複」策略 regex 會把合理下一步改善連殺九次，已移除該模糊裁決，只保留明確「提示錯／偏保守」反轉紅線。之後仍重跑 schema、罐頭、L4、Game FSM、明確 Hint 反轉；Game Debrief 全部可見拆盤欄位一併納入。失敗不落罐頭、不扣費、不計次。本輪 runtime **960/960**。高風險審查：`docs/reviews/2026-07-14-practice-grounding-repair-codex-review.md`；修正版 Edge production smoke 待部署後補。
