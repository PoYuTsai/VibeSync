...
- **(a) client 形狀驗證器（env-free，可純 TDD）**：reframer.ts 的 `CLIENT_RECORD_FIELD_SHAPES`／`REPLY_OPTION_FIELD_SHAPES`／`ClientFieldShape` type 加 `export`（純附加、行為中性，跑全套 Deno 證零回歸即可，不算高風險行為改動）。新模組 `client_shape_validator.ts` 寫純函式 `findClientShapeViolations(finalResult)`：對 finalResult 每個有 shape 表的 key 驗 JSON type（string／boolean／int=Number.isInteger／number／stringArray／record／recordArray；**undefined 與 null 放行**＝client 是 nullable cast，只有「present 且非 null 且錯型」才算 violation）。CLI `tools/voice-benchmark/check_client_shape.ts` 讀 baseline ndjson、取 `analysis.done .finalResult`（＋ reply_option 走 REPLY_OPTION 表）、有 violation 退非零。**TDD 用合成 fixture**（good＝全合規；bad＝`warnings:"x"`／`enthusiasm.score:72.5`／`psychology.shitTest:"x"`／`dimensions.heat:"hot"`／`recognizedConversation.messages:[1,2]`），**絕不用既有 baselines 當 good fixture**——gate4 等 baseline 是 2026-06-12 守門前產物（實見 `finalResult.psychology` 為裸字串、與表 record 形狀衝突＝守門前洩漏），會假性 violation。
