...
Status: **OPEN — dogfood FAILED（Eric 2026-06-13，同張 golden 圖 ≥4 球仍只出 2 球）：三件 prompt 改動在目標案例上未生效，雙審 P3 成真**。⚠️ 但屬「加指令、模型不理＝行為同改前」＝**非新 regression（只是沒改善），留著 deployed 與改前一樣安全、不急 revert**。下一步（新 session，高風險 AI 行為除錯，先黑箱不臆測）：用測試帳號 curl 打 prod stream（手法見 memory `p0-stream-reply-option-fix-2026-06-12`，payload 用 `tools/voice-benchmark/cases/golden_anchor_recon.json`），看 raw stream 的 reply_option/finalResult **段數**，分辨兩假設——①**模型不聽**：盤點/下限壓不過併球慣性（解＝強化指令或換 few-shot 範例，非再加規則散文）；②**server 丟段**：模型出 ≥3 段但 post_process/reframer contract 砍成 2（#12/P0 老路徑、server 丟段零 telemetry，解＝補 log-only telemetry 先驗屍再修 contract）。動 contract 屬高風險須雙審。
