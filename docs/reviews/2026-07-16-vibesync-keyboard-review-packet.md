...
1. Auth：App Group Keychain 只共享 access token／userId／expiry；不共享 refresh token。事件寫入序列化、登出先刪 token。
...
5. App Review：未開完整取用仍有 ABC／標點／空白／換行／退格與地球切換；extension 內無購買流程。
...
- `flutter test test/unit/core/services/keyboard_token_bridge_test.dart test/widget/screens/keyboard_setup_screen_test.dart test/widget/screens/settings_screen_test.dart test/widget/router_test.dart` → 29 passed
...
- Apple Developer Portal 必須建立 `group.com.poyutsai.vibesync`，掛到主 App 與 `com.poyutsai.vibesync.keyboard`，並建立／更新 provisioning profile。
