...
## 13. 內容驗收補修（真模型內容未過→本機限定補修＋離線回歸）— 2026-09-18

驗收複核裁決：真 Postgres 並行結案；工程 APPROVE_WITH_RISK 維持；**真模型內容驗收未過，暫不進正式部署**。reviewer 給 27 筆離線標籤（Eric 貼入；reviewer 未執行 handler 測試）。本輪只改 Edge 內容層（`opener_material／opener_flow_payload／opener_flow_handler／opener_flow_prompt`），不動 DB／Flutter／B／C／P2；未跑第二批付費模型。

- **回歸方法**：`tools/opener-content-replay`——27 筆捕獲的 snapshot＋contribution＋raw 注入正式 validate→materials→normalize→硬檢查（含採用）→projection（Free／paid 各一次）；labels 為 expect_flag／expect_clean／adoption。先紅：`ccbabe98` 51/97（`red_replay.txt`）；修後 97/97。
- **A 生成內容與推薦忠實性**：新硬檢查 `sender_fact_transposed`（經歷寫進沒有「我」的句子）、`sender_fact_extended`（加上沒說的健康／人生事件）、`relative_quote_fabricated`（家人被加引語）、`certainty_upgraded`（提過想／還沒→說好／答應）、`profile_fact_reversed`（自介早睡↔晚睡）；原料採用 `checkMaterialAdoption`／`cardAdoptsMaterial`：內容雙字證據、不看模型自稱 references、被標記的卡不算採用、純否定／排除補充不要求、想約目標要帶輕邀約〔2026-09-24 取代：照片／自介入口只做開場教練，五張卡都不提出邀約，想約目標改從 X 開話題、本身不要求採用；用戶目標子句的目標字眼（想約、約她、一起去、見面…）與「一起」不算採用證據，目標子句裡其他字（X、週末、有空…）仍是字面證據，不是邀約偵測；見 `docs/decisions.md` ADR #47 附錄，本段歷史不改寫〕；沒接住→`material_unused` 標在排序第一的可見卡交一次修正，修不好 502 不扣不計次（新增 E2E）。投影推薦先從有證據的可見卡依 rankedPicks 取，不再一律 extend；`matched` 要證據＋來源。prompt 補對應規則、修正提示對映新碼。
- **B 硬檢查誤報／漏報**：冒號／引號後的代言（柴犬：我養妳）不算自述、「我懂」不算；正反問句（還有沒有推薦）不是否定；「下班不聊工作吧」是遵守不是使用；「不聊工作」延伸到自介職業線索（美容師／獸醫助理）才算使用。沒有刪保護、沒有按情境加例外。
- **保留**：五卡／Free 三卡契約、一次額外修復上限、deadline 與計費邊界；版本常數（進 hash）不動。
- **確定性守門做不到的**（如實）：程度擴張（「自己選路」→「完全不聽指揮」）、一般性的細節新增，只靠 prompt 規則與盲審。
- **新 prompt 是否變好需新輸出確認**：固定小集合＝受影響 7 情境（multi-hook／family-fact／filter-heavy／she-said-vs-guess／single-hook-dog／goal-not-consent／real-life-scene）×A／B／略過×3、不含舊單段，dry-run 估 **70 次≈$1.64**（未執行；原 $5 為一次性授權，需另授權）。
- 測試：analyze-chat Deno 全套（見 `r6_deno_analyze_chat_full.*`）；Flutter 未重跑（本輪無 Flutter 改動）。
