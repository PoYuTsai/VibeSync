...
- **A 生成內容與推薦忠實性**：新硬檢查 `sender_fact_transposed`（經歷寫進沒有「我」的句子）、`sender_fact_extended`（加上沒說的健康／人生事件）、`relative_quote_fabricated`（家人被加引語）、`certainty_upgraded`（提過想／還沒→說好／答應）、`profile_fact_reversed`（自介早睡↔晚睡）；原料採用 `checkMaterialAdoption`／`cardAdoptsMaterial`：內容雙字證據、不看模型自稱 references、被標記的卡不算採用、純否定／排除補充不要求、想約目標要帶輕邀約；沒接住→`material_unused` 標在排序第一的可見卡交一次修正，修不好 502 不扣不計次（新增 E2E）。投影推薦先從有證據的可見卡依 rankedPicks 取，不再一律 extend；`matched` 要證據＋來源。prompt 補對應規則、修正提示對映新碼。
