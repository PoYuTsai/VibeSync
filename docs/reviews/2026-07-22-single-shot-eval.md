...
## 第 7 輪（2026-07-23；第 7 輪前修復清單 1–5 全數落地後首跑）

修復 commit：`9fd3b8a5`（temperature 去列字第二注入點）、`d63628c0`（gameBreakdown 拍平 reparent repair）、`dde9fbbb`（GENERIC_PROPOSAL 構詞守門）、`dc37a2c2`（P5 收尾局 steady 放寬 direct＝裁決 b）、`a986adba`＋`294dc9c7`（grounding 三型窄豁免＋質問應對教學＝裁決 a）、`02608725`（fact-ledger 邊角七項）、`7c09591a`（「投入度 X/100」分數形守門補洞，final review 抓的）。全套 1010 測試綠。
結果 JSON：`results/2026-07-23T04-47-01-941Z.json`

| 路徑 | p50 | p90 | 首發（gate ≥95%） | 503（gate 0） | 對比第 6 輪 |
|------|-----|-----|------|------|------|
| 新手 hint | 5.8s | 7.3s | **100% ✅** | **0 ✅** | 85%→100%、invite_route FP 歸零 |
| Game hint | 7.5s | 12.3s | 80% ❌ | 1 ❌ | 70%→80%、503 3→1 |
| 新手 debrief | 8.1s | 11.2s | 90% ❌ | 1 ❌ | **95%→90% 回退**、503 0→1 |
| Game debrief | 11.1s | 12.7s | **95% ✅** | **0 ✅** | 60%→95%、503 4→0、p90 21.2→12.7（拍平 repair 生效） |

詞表洩漏軸持續 0 ✅。速度軸四路全綠。殘餘打回分佈（6 筆）：
- game_hint：not_grounded×3、invite_route×1、venue:located_at×1
- beginner_debrief：third_party:name:is_named×1、**temperature_leak×1（去列字後仍現，待逐筆看漏詞）**、user:preference:likes×1
- game_debrief：third_party:name:is_named×1（次發即救回，未成 503）

### 第 7 輪判定結論（待逐筆重放）

- breakdown_missing 歸零（repair 生效）、新手 hint 全綠、game debrief 轉綠——第 6 輪三大家族全數收掉。
- 仍紅兩路共 8 筆打回、2×503；下一步照慣例逐筆重放判定 TP/FP（重點：temperature_leak 殘 1 筆的漏詞是什麼、not_grounded×3 是否為三型豁免未涵蓋的新形態、beginner_debrief 回退是雜訊還是新 FP 家族）。
