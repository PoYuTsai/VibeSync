...
**Step 5: Migrate 13 conversation repo write 呼叫點 → controller（unit = repo write site, not invalidate site）**

**Unit-of-analysis 校正（r4 fix per Codex r3 verdict）**：r3 migration table 用「invalidate site」當清單單位漏抓 3 處（`analysis_screen.dart:541,613,649` — 它們 update 後只 invalidate `conversationProvider(id)` per-id，沒 global invalidate，剛好被 invalidate-site grep 漏網）。r4 改用「repo write site」當單位，覆蓋 100% 的 conversation 資料變更路徑。

**Conversation repo write sites（13 處 grep verified 2026-04-26）→ 全部改走 controller：**

| File | Line | Op | Original invalidate（若有） | Migration |
|---|---|---|---|---|
| `main_shell.dart` | 241 | create | line 245 global | `controller.create(...)` |
| `new_conversation_screen.dart` | 127 | create | line 146 global | `controller.create(...)` |
| `new_conversation_screen.dart` | 144 | update | line 146 global | `controller.save(c)` |
| `home_screen.dart` | 90 | delete | line 91 global | `controller.delete(c)` |
| `analysis_screen.dart` | 494 | update | line 495 global | `controller.save(c)` |
| `analysis_screen.dart` | 513 | delete | line 514 global | `controller.delete(c)` |
| `analysis_screen.dart` | **541** | update | line 542 per-id only | `controller.save(c)` ← **r4 補** |
| `analysis_screen.dart` | **613** | update | line 614 per-id only | `controller.save(c)` ← **r4 補** |
| `analysis_screen.dart` | **649** | update | line 650 per-id only | `controller.save(c)` ← **r4 補** |
| `analysis_screen.dart` | 920 | create | line 935 global | `controller.create(...)` |
| `analysis_screen.dart` | 934 | update | line 935 global | `controller.save(c)` |
| `analysis_screen.dart` | 999 | update | line 1000 global | `controller.save(c)` |
| `analysis_screen.dart` | 1110 | update | line 1113 global | `controller.save(c)` |

> 遷移後 controller 內部統一處理：repo write + `_invalidatePartnerScope` + `_invalidateLegacyGlobal`。原本的 per-id `ref.invalidate(conversationProvider(widget.conversationId))` 行可保留（它們不衝突，仍對 `conversationProvider.family` 單筆 refresh 有用）；或移除若已被 controller 邏輯涵蓋。**Implementation 階段每處 case-by-case 判斷**。

**Session-scope invalidate sites（4 處 — 不對應 repo write）→ 保持原樣，**不**走 controller：**
...
**Verification gate（r4 strengthened — primary gate 改為 repo write coverage）**:

1. **Repo write coverage（primary gate, r4 added per Codex r3）**:
   ```bash
   grep -rn 'repository\.\(create\|update\|delete\)Conversation' lib/ \
     | grep -v 'conversation_repository\.dart' \
     | grep -v 'conversation_write_controller\.dart'
   ```
   → 期望 **0 hits**。若仍有殘存，**逐一**列在 plan 並標明保留理由（不該有正當 exception；A2 假設 100% conversation 寫入經 controller）。

2. **Global invalidate coverage（secondary gate）**:
   `grep -rn "ref.invalidate(conversationsProvider)" lib/` → 期望 **5 hits**：4 個 session-scope（保留）+ 1 個 controller 內部（`_invalidateLegacyGlobal()`）

3. **Analyze**: `flutter analyze` → 0 warnings

4. **Manual smoke**: home → partner detail → analyze → My Report flow，所有 UI 即時更新（特別檢查 partner aggregate 在 message 編輯 / 刪除後即時更新——這正是 r3 漏抓的 541/613/649 場景）

**Step 6: Commit (split 2 commits)**

```bash
git commit -m "[feat] Partner narrow-invalidation providers + ConversationWriteController"
git commit -m "[refactor] 13 conversation repo write 呼叫點改走 ConversationWriteController"
git push
```
