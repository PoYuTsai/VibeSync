...
這一輪修完後，如果以下條件都成立，就可以先停下來 build TestFlight：

- [ ] OCR 前置預檢能先擋掉明顯不是聊天截圖的圖片。
- [ ] OCR / 分析流程中，不再直接露出技術術語或英文錯誤。
- [ ] A / B / C1 / C2 至少各跑過一輪，沒有新的 P1 問題。

如果上面 3 項都成立，這版就可以先進入你和夥伴的實機回歸，不用再無限往下修。
...
| 情境 | 原始大小 | 壓縮後大小 | Payload | Round-trip | AI latency | 分類/方向 | 結果 |
|------|----------|------------|---------|------------|------------|-----------|------|
| 正常單圖 | | | | | | | |
| 長篇繁中圖 | | | | | | | |
| LINE 回覆圖 | | | | | | | |
...
- `分類/方向` 建議記成：例如 `valid_chat / 高 / 待確認 1 / 引用併回 2`，方便定位是 OCR 結構問題還是純延遲問題。
