...
常見 `failureClass`：`fal_image_http_<狀態碼>`（供應商回錯，402/403 多半是額度或 key）、`fal_image_too_small`（黑圖保險）、`fal_image_too_large`（超過 12MB 上限）、`fal_image_bad_magic`（位元組檔頭不是 JPEG 也不是 PNG；錯誤頁與錯誤 JSON 也在這裡拒收——格式認定只看位元組，header 沒有否決權）、`fal_image_download_failed`、`fal_image_timeout`（fal 呼叫超過 60s）、`fal_image_upload_timeout`。
