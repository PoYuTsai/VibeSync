    ...
    return SingleChildScrollView(
        ...
        child: Column(
            ...
            children: [
              ...
              Text(
                  widget.newTopic
                      ? '每次成功生成使用 3 則額度。回看與複製已取得的話題不另扣則；相同作業的重試不重複扣費。'
                      : widget.legacy
                          ? '目前使用一般生成：每次成功生成開場白使用 3 則額度。'
                          : '目前會話在第一次成功生成回覆時使用 3 則額度，這一局包含 3 組。後續生成依這一局剩餘權益處理。',
                  style: OpenerHomeStyle.body),
              const SizedBox(height: 16),
              if (_loading || usage.isLoading)
                ...
                Text(
                    ...
                    style: OpenerHomeStyle.body),
                TextButton(onPressed: _refresh, child: const Text('更新額度')),
              ],
            ]));
  }
}
