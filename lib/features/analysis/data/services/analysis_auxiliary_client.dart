...
export '../../domain/entities/analysis_telemetry.dart';
...
/// AnalyzeChat 輔助流程的 typed client：截圖辨識（免費 OCR）、草稿潤飾、
/// 回覆微調。三個入口共用同一條 `_analyze` 傳輸核心（重試策略、錯誤對映、
/// 進度里程碑與 telemetry 逐字沿用拆分前行為）。主分析不在這裡——
/// 它只走 AnalyzeStreamClient 的 NDJSON 串流。
class AnalysisAuxiliaryClient {
  ...
  /// 免費 OCR：一批 1–3 張截圖辨識成結構化對話。不扣訂閱額度、
  /// 絕不自動重送（單次 HTTP）。
  Future<AnalysisResult> recognizeScreenshots({
  ...
  /// 草稿潤飾（optimize-message 計費路徑）。[requestId] 是 exactly-once
  /// 帳本的 durable 身分；缺席時鑄一顆一次性 id（測試便利，production
  /// 一律由 OptimizeRequestRunner 供給）。
  Future<AnalysisResult> optimizeDraft({
  ...
  /// 回覆微調：同一條 optimize-message 計費路徑，多帶指令與多輪漂移錨。
  Future<AnalysisResult> refineReply({
    required List<Message> messages,
    required String userDraft,
  ...
  Future<AnalysisResult> _analyze(
    ...
    analysisDebugLog('[AnalysisAuxiliaryClient] analyze start');
    analysisDebugLog(
      '[AnalysisAuxiliaryClient] messages: ${sanitizedMessages.length}, images: ${images?.length ?? 0}, recognizeOnly: $recognizeOnly',
    );

    for (var attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        analysisDebugLog(
          '[AnalysisAuxiliaryClient] attempt ${attempt + 1}/${maxRetries + 1}',
        );
        ...
        analysisDebugLog(
          '[AnalysisAuxiliaryClient] attempt ${attempt + 1} failed: ${error.runtimeType} - $error',
        );
