...
- Security rounds 2-4 are done: `auth_diagnostics` now has schema guardrails plus a dedicated ingestion function, observability retention is scheduled with `pg_cron`, active security signals are queryable from Supabase/admin dashboard, and the incident-response runbook is part of the launch docs.
...
> **新 Session 入口文件** - Codex 開始時自動讀取此檔案
...
3. ✅ **解決**: 使用 `flutter_web_auth_2` 實現 ASWebAuthenticationSession (像 Codex app)
...
| **截圖上傳功能** | ✅ 完成 | Codex Vision 識別對話、最多 3 張、自動壓縮 |
...
- **Codex Model**: `Codex-sonnet-4-20250514` (Essential) / `Codex-haiku-4-5-20251001` (Free/Starter)
...
- **AI**: Codex API (Haiku + Sonnet 混合策略)
...
> **重要**: Codex 遇到 bug 時必須自動更新此文件，不需要用戶提醒

### 自動記錄流程
```
Bug 發生 → 分析 → 修復 → 寫測試 → 更新 AGENTS.md → commit & push
```
...
> **重要**: Codex API 不會從單次呼叫中「學習」，優化是透過改進 System Prompt
...
#### [2026-02-28] Codex 模型名稱過期
**症狀**: Edge Function 返回 "model not found" 錯誤
**Root Cause**: Codex 3.5 模型已停用，需改用 Codex 4.x
**修復**:
- `Codex-3-5-haiku-20241022` → `Codex-haiku-4-5-20251001`
- `Codex-sonnet-4-20250514` 保持不變
...
**修復**:
使用 `flutter_web_auth_2` 套件實現 ASWebAuthenticationSession（像 Codex app 一樣）
```dart
...
#### [2026-03-12] 截圖上傳功能 - Codex Vision
**決定**: 新增截圖上傳功能，使用 Codex Vision API 識別對話內容
...
- **新 Session**: 讀此文件 (`AGENTS.md`) 即可了解專案全貌
