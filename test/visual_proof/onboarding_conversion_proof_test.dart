...
void main() {
  ...
  testWidgets('onboarding conversion — all nine pages', (tester) async {
    ...
    for (var page = 2; page <= 9; page++) {
      ...
      // 問卷頁（第 7 頁）鏡像卡三態：未選（上面那張）→ 選 1 → 選 2。選擇保留著
      ...
      if (page == 7) {
        ...
        await capture('onboarding_page7_selected1.png');

        await tester
            .tap(find.byKey(const ValueKey('onboarding-goal-comfortableChat')));
        await tester.pumpAndSettle();
        await capture('onboarding_page7_selected2.png');
      }
    }
  });
}
