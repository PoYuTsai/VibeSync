...
void main() {
  ...
  PracticeChatState debriefingSeed() {
    final girl = practiceGirlProfiles.first;
    return PracticeChatState(
      sessionId: 'debrief-wait-progress-test',
  ...
  Future<_SeededPracticeChatController> pumpDebriefing(
    WidgetTester tester,
  ) async {
  ...
  testWidgets('拆解等待依時間切換三段文案並顯示經過秒數；載入結束 timer 收斂', (tester) async {
    final controller = await pumpDebriefing(tester);
    final progressRow =
        find.byKey(const ValueKey('practice-debrief-wait-progress'));

    // 0-10s：第一段文案＋經過秒數。
    expect(progressRow, findsOneWidget);
    expect(find.textContaining('教練正在回顧整局對話'), findsOneWidget);
    expect(find.textContaining('0 秒'), findsOneWidget);

    // 9s：仍是第一段。
    await tester.pump(const Duration(seconds: 9));
    expect(find.textContaining('教練正在回顧整局對話'), findsOneWidget);
    expect(find.textContaining('9 秒'), findsOneWidget);

    // 10s：切第二段。
    await tester.pump(const Duration(seconds: 1));
    expect(find.textContaining('正在整理亮點和可以更好的地方'), findsOneWidget);
    expect(find.textContaining('教練正在回顧整局對話'), findsNothing);
    expect(find.textContaining('10 秒'), findsOneWidget);

    // 25s：切第三段。
    await tester.pump(const Duration(seconds: 15));
    expect(find.textContaining('快好了，正在做最後檢查'), findsOneWidget);
    expect(find.textContaining('正在整理亮點和可以更好的地方'), findsNothing);
    expect(find.textContaining('25 秒'), findsOneWidget);

    // 載入結束：進度列消失、timer 必須取消 → pumpAndSettle 必收斂。
    controller.emit(
      controller.currentState.copyWith(isDebriefing: false),
    );
  ...
  testWidgets('等待中重新載入 → 秒數與文案從第一段重新起算', (tester) async {
    ...
    // 結束再重新進入拆解中狀態 → 從 0 秒第一段重來。
    controller.emit(controller.currentState.copyWith(isDebriefing: false));
    await tester.pumpAndSettle();
    controller.emit(controller.currentState.copyWith(isDebriefing: true));
    await tester.pump();
